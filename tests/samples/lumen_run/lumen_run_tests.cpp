...
void testLumenLearningProofVerticalSlice() {
  const std::filesystem::path project = repoPath("projects/lumen_run/lumen_run.asterproj");
  aster::LearningSessionLoadResult loaded =
      aster::loadLearningSession(project, "lesson.lumen_mining");
  assert(loaded.ok());
  const std::filesystem::path output =
      std::filesystem::temp_directory_path() / "aster_lumen_learning_vertical_slice";
  std::filesystem::remove_all(output);
  const aster::LumenLearningProofResult proof =
      aster::runLumenMiningLearningProof({}, std::move(loaded.session), output);
  assert(proof.passed);
  assert(proof.report.evaluators_consistent);
  assert(proof.report.authoring.objective_coverage == 1.0f);
  assert(proof.report.authoring.workflow_coverage == 1.0f);
  assert(std::filesystem::exists(proof.artifacts.trace_path));
  assert(std::filesystem::exists(proof.artifacts.report_path));
  std::filesystem::remove_all(output);
}
...
int main(const int argc, const char **argv) {
  const NamedSampleTest tests[] = {
      ...
      {"lumen_learning_proof_vertical_slice", testLumenLearningProofVerticalSlice},
