    ...
    @autoreleasepool {
      ...
      NSString *source =
          @"#include <metal_stdlib>\n"
           ...
           "float3 biological_integument(float3 base, constant Object &object, float3 world, "
           ...
           "float capillary = projected_fbm(world + float3(0.07, 0.13, -0.05), normal, detail * "
           "0.82, 467.0); "
           "float pore = ridge1(projected_fbm(world, normal, detail * 2.20, 479.0)); "
           "float abrasion = ridge1(projected_fbm(world + normal * 0.09, normal, detail * 1.48, "
           "491.0)); "
           "float follicle = 0.5 + 0.5 * sin((world.z * object.pattern_params.z + world.x * "
           "object.pattern_params.y * 0.32 + uv.x * 7.0) * 3.35 + pigment * 4.0); "
           "float follicle_mask = smooth1(0.50, 0.94, follicle) * smooth1(0.16, 0.84, pore + "
           "object.pattern_params.w); "
           "float low_pelage = smooth1(0.60, 0.94, normal.y) * smooth1(0.42, 0.98, abs(world.z) * "
           "0.35 + abs(world.x) * 0.20); "
           "float vascular = smooth1(0.48, 0.96, capillary) * saturate1(0.20 + "
           "object.pattern_params.w * 1.50 + object.procedural_params.w * 0.75); "
           "float pigment_gain = 0.35 + object.pattern_params2.x * 0.65 + "
           "object.procedural_params.x * 0.20; "
           "float3 color = mix(base * float3(0.72, 0.63, 0.52), base * float3(0.46, 0.38, 0.28), "
           "pigment * pigment_gain); "
           "color = mix(color, float3(0.58, 0.23, 0.17), vascular * (0.32 + low_pelage * 0.28)); "
           "color = mix(color, base * float3(1.15, 1.03, 0.78), follicle_mask * (0.30 + "
           "object.material_params.z * 0.16)); "
           "color = mix(color, color * float3(0.72, 0.68, 0.60), smooth1(0.74, 0.98, abrasion) * "
           "(0.10 + object.pattern_params2.x * 0.38)); "
           "color = mix(color, float3(0.20, 0.18, 0.15), smooth1(0.82, 0.99, pore) * 0.10); "
           "return clamp(color * (0.92 + pore * 0.10), float3(0.0), float3(4.0)); }\n"
           ...
           "if (is_pattern(pattern, 19.0) > 0.5) return apply_procedural_layer("
           "biological_integument(base, object, world, normal, uv), object, world, normal); "
...
private:
  ...
  MetalObjectUniforms makeShadowObjectUniforms(const aster::RenderObject &object,
    ...
    const aster::Material material =
        aster::applyWorldPerceptualMaterialMemory(object, object.material);
  ...
  MetalObjectUniforms makeObjectUniforms(const aster::RenderObject &object,
    ...
    std::memcpy(out.model_view_projection, mvp.m.data(), sizeof(out.model_view_projection));
    std::memcpy(out.normal_matrix, normal_matrix.m.data(), sizeof(out.normal_matrix));
    out.base_color_opacity[0] = material.base_color.x;
    out.base_color_opacity[1] = material.base_color.y;
    out.base_color_opacity[2] = material.base_color.z;
    out.base_color_opacity[3] = opacity;
    out.emission_strength[0] = material.emission_color.x;
    out.emission_strength[1] = material.emission_color.y;
    out.emission_strength[2] = material.emission_color.z;
    out.emission_strength[3] = material.emission_strength;
    out.material_params[0] = material.roughness;
    out.material_params[1] = material.metallic;
    out.material_params[2] = material.detail_strength;
    out.material_params[3] = material.detail_scale;
    ...
    out.pattern_params[1] = material.pattern_scale.x;
    out.pattern_params[2] = material.pattern_scale.y;
    out.pattern_params[3] = material.pattern_depth;
    out.pattern_params2[0] = material.pattern_contrast;
    out.pattern_params2[1] = material.pattern_mortar;
    out.pattern_params2[2] = material.ambient_occlusion;
    out.pattern_params2[3] = material.double_sided ? 1.0f : 0.0f;
    out.procedural_params[0] = material.procedural.macro_variation;
    out.procedural_params[1] = material.procedural.micro_normal_strength;
    out.procedural_params[2] = material.procedural.roughness_variation;
    out.procedural_params[3] = material.procedural.wetness;
    out.procedural_params2[0] = material.procedural.height_shading;
