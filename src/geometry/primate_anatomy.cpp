...
[[nodiscard]] int integumentLayerCount(const CercopithecidaeMorphologySpec &spec) {
...
[[nodiscard]] AnatomicalModelPart furSkinEnvelope(const CercopithecidaeMorphologySpec &spec,
  ...
  report.integument.epidermal_layer_count = integumentLayerCount(spec);
  report.integument.epidermal_thickness_proxy =
      (0.006f + std::max(spec.integument_shell_offset, 0.0f) * 0.38f) * spec.scale;
  report.integument.dermal_elasticity =
      std::clamp(0.58f + spec.tension_line_strength * 0.12f, 0.0f, 1.0f);
  report.integument.hypodermal_vascularity = 0.18f + capillary * 0.74f;
  report.integument.pigment_heterogeneity = pigment;
  report.integument.follicle_guide_count = guide_count;
  report.integument.gland_cluster_count = gland_count;
  report.integument.capillary_translucency = capillary;
  report.integument.micro_abrasion_density =
      std::clamp(0.18f + spec.surface_detail_strength * 0.34f + pigment * 0.16f, 0.0f, 1.0f);
  report.integument.dynamic_tension_line_count = tension_lines;
  report.integument.low_pelage_zone_ratio = 0.24f;
  report.integument.plantar_integument_thickening = 0.024f * spec.scale;
  ...
  part.landmarks.push_back({"integument.epidermal_layers",
                            scaled({0.0f, 1.56f, 0.70f}, spec.scale),
                            static_cast<float>(report.integument.epidermal_layer_count)});
  part.landmarks.push_back({"integument.dermal_elasticity",
                            scaled({0.22f, 1.42f, 0.50f}, spec.scale),
                            report.integument.dermal_elasticity});
  part.landmarks.push_back({"integument.hypodermal_vascularity",
                            scaled({0.0f, 0.91f, -0.82f}, spec.scale),
                            report.integument.hypodermal_vascularity});
  part.landmarks.push_back({"integument.pigment_heterogeneity",
                            scaled({0.0f, 1.20f, -0.20f}, spec.scale),
                            report.integument.pigment_heterogeneity});
  part.landmarks.push_back({"integument.gland_clusters",
                            scaled({0.12f, 1.44f, 0.76f}, spec.scale),
                            static_cast<float>(gland_count)});
  part.landmarks.push_back({"integument.capillary_translucency",
                            scaled({0.18f, 1.58f, 0.57f}, spec.scale),
                            report.integument.capillary_translucency});
  part.landmarks.push_back({"integument.micro_abrasion_density",
                            scaled({0.0f, 1.02f, -0.35f}, spec.scale),
                            report.integument.micro_abrasion_density});
  part.landmarks.push_back({"integument.dynamic_tension_lines",
                            scaled({0.0f, 1.58f, 0.58f}, spec.scale),
                            static_cast<float>(tension_lines)});
  part.landmarks.push_back({"integument.low_pelage_zones",
                            scaled({0.0f, 1.43f, 0.82f}, spec.scale),
                            report.integument.low_pelage_zone_ratio});
...
AnatomicalModel makeCercopithecidaeModel(CercopithecidaeMorphologySpec spec) {
  if (spec.scale <= 0.0f || spec.surface_segments < 8 || spec.surface_rings < 4 ||
      spec.integument_epidermal_layers < 1 || spec.follicle_density < 0.0f ||
      spec.gland_cluster_count < 0) {
