...
struct plane_t {
    aster_linear::vec3 normal;
    float     offset;
};

struct ray_t {
    aster_linear::vec3 origin;
    aster_linear::vec3 direction;
};
...
struct ray_hit_t {
    ...
    fragment_t *fragment;
    float      parameter;
    aster_linear::vec3  position;
};

struct box_t {
    aster_linear::vec3 min, max;
};
...
struct world_t {
    ...
    std::vector<brush_t*>  query_point(const aster_linear::vec3& point);
    std::vector<brush_t*>  query_box(const box_t& box);
    std::vector<ray_hit_t> query_ray(const ray_t& ray);
    std::vector<brush_t*>  query_frustum(const aster_linear::mat4& view_projection);
    std::any               userdata;
