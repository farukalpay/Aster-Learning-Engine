...
#include <aster_linear/gtc/matrix_access.hpp>
// #include <aster_linear/gtx/string_cast.hpp>
...
static float signed_distance(const aster_linear::vec3& point, const plane_t& plane) {
    return aster_linear::dot(plane.normal, point) + plane.offset;
}

static box_t extended(const box_t& box, const aster_linear::vec3& point) {
    return box_t{
        aster_linear::min(box.min, point),
        aster_linear::max(box.max, point)
    };
}

// does brush0 come before brush1 in the solid_ops order?
...
static relation_t test(fragment_t* fragment, face_t* face) {
    ...
    if (count[RELATION_OUTSIDE] > 0 &&
        ...
        return RELATION_SPLIT;
             ...
             count[RELATION_INSIDE] == 0) {
        float d = aster_linear::dot(face->plane->normal, fragment->face->plane->normal);
        if (d < 0)
...
static bool try_make_vertex(face_t *f0, face_t *f1, face_t *f2, vertex_t& v) {
    ...
    aster_linear::mat3 m;
    m = aster_linear::row(m, 0, f0->plane->normal);
    m = aster_linear::row(m, 1, f1->plane->normal);
    m = aster_linear::row(m, 2, f2->plane->normal);

    // printf("m = %s\n", aster_linear::to_string(m).c_str());
    // fflush(stdout);

    float D = aster_linear::determinant(m);
    // printf("D = %f\n", D); fflush(stdout);

    if (approx_equal(D, 0.0f))
        return false;
    aster_linear::mat3 mx(m), my(m), mz(m);
    aster_linear::vec3 offsets(f0->plane->offset, f1->plane->offset, f2->plane->offset);
    // printf("offsets = %s\n", aster_linear::to_string(offsets).c_str());
    // fflush(stdout);

    mx = aster_linear::column(mx, 0, -offsets);
    my = aster_linear::column(my, 1, -offsets);
    mz = aster_linear::column(mz, 2, -offsets);

    // printf("mx = %s\n", aster_linear::to_string(mx).c_str());
    // fflush(stdout);
    // printf("my = %s\n", aster_linear::to_string(my).c_str());
    // fflush(stdout);        
    // printf("mz = %s\n", aster_linear::to_string(mz).c_str());
    // fflush(stdout);    

    v.faces[0] = f0;
    v.faces[1] = f1;
    v.faces[2] = f2;
    v.position = aster_linear::vec3(
        aster_linear::determinant(mx) / D,
        aster_linear::determinant(my) / D,
        aster_linear::determinant(mz) / D
    );  
    return true;
}
...
static void fix_winding(face_t* face) {
    ...
    if (face->vertices.size() < 3)
        return;
    aster_linear::vec3 v0 = face->vertices[0].position;
    aster_linear::vec3 v1 = face->vertices[1].position;
    aster_linear::vec3 v2 = face->vertices[2].position;
