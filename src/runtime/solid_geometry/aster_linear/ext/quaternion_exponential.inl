...
namespace aster_linear
{
	...
	ASTER_LINEAR_FUNC_QUALIFIER qua<T, Q> exp(qua<T, Q> const& q)
	{
		vec<3, T, Q> u(q.x, q.y, q.z);
		T const Angle = aster_linear::length(u);
		if (Angle < epsilon<T>())
			return qua<T, Q>();
	...
	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER qua<T, Q> log(qua<T, Q> const& q)
	{
		vec<3, T, Q> u(q.x, q.y, q.z);
		T Vec3Len = length(u);
	...
	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER qua<T, Q> pow(qua<T, Q> const& x, T y)
	{
	...
	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER qua<T, Q> sqrt(qua<T, Q> const& x)
	{
