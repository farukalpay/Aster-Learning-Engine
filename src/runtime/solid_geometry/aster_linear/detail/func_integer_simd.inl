...
namespace detail
{
	...
	{
		ASTER_LINEAR_FUNC_QUALIFIER static vec<4, uint, Q> call(vec<4, uint, Q> const& v, uint Mask, uint Shift)
		{
...
#	if ASTER_LINEAR_ARCH & ASTER_LINEAR_ARCH_AVX_BIT
	template<>
	ASTER_LINEAR_FUNC_QUALIFIER int bitCount(uint x)
	{
		return _mm_popcnt_u32(x);
	}

#	if(ASTER_LINEAR_MODEL == ASTER_LINEAR_MODEL_64)
	template<>
	ASTER_LINEAR_FUNC_QUALIFIER int bitCount(detail::uint64 x)
	{
