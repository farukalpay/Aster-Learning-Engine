...
namespace aster_linear
{
	...
	ASTER_LINEAR_FUNC_QUALIFIER void axisAngle(mat<4, 4, T, Q> const& m, vec<3, T, Q>& axis, T& angle)
	{
		T const epsilon =
        ...
        axis = aster_linear::normalize(aster_linear::vec<3, T, Q>(
            m[1][2] - m[2][1], m[2][0] - m[0][2], m[0][1] - m[1][0]));
	}

	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER mat<4, 4, T, Q> axisAngleMatrix(vec<3, T, Q> const& axis, T const angle)
	{
		T c = cos(angle);
		T s = sin(angle);
	...
	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER mat<4, 4, T, Q> extractMatrixRotation(mat<4, 4, T, Q> const& m)
	{
		return mat<4, 4, T, Q>(
	...
	template<typename T, qualifier Q>
	ASTER_LINEAR_FUNC_QUALIFIER mat<4, 4, T, Q> interpolate(mat<4, 4, T, Q> const& m1, mat<4, 4, T, Q> const& m2, T const delta)
	{
