...
#ifndef ASTER_LINEAR_HAS_CXX11_STL
#include <algorithm>
#else
#include <utility>
#endif
...
namespace aster_linear {
	...
	namespace _internal_
	{
		...
		template<typename T>
		ASTER_LINEAR_INLINE T transferSign(T const& v, T const& s)
		{
			return ((s) >= 0 ? aster_linear::abs(v) : -aster_linear::abs(v));
		}

		template<typename T>
		ASTER_LINEAR_INLINE T pythag(T const& a, T const& b) {
			static const T epsilon = static_cast<T>(0.0000001);
			T absa = aster_linear::abs(a);
			T absb = aster_linear::abs(b);
			if(absa > absb) {
				absb /= absa;
				absb *= absb;
				return absa * aster_linear::sqrt(static_cast<T>(1) + absb);
			}
			if(aster_linear::equal<T>(absb, 0, epsilon)) return static_cast<T>(0);
			absa /= absb;
			absa *= absa;
			return absb * aster_linear::sqrt(static_cast<T>(1) + absa);
		}
	...
	)
	{
		...
		for(i = D; i >= 2; i--)
		{
			...
			if(l > 1)
			{
				for(k = 1; k <= l; k++)
				{
					scale += aster_linear::abs(a[(i - 1) * D + (k - 1)]);
				}
				if(aster_linear::equal<T>(scale, 0, epsilon))
				{
					e[i - 1] = a[(i - 1) * D + (l - 1)];
				}
				else
				{
					...
					f = a[(i - 1) * D + (l - 1)];
					g = ((f >= 0) ? -aster_linear::sqrt(h) : aster_linear::sqrt(h));
					e[i - 1] = scale * g;
					h -= f * g;
					a[(i - 1) * D + (l - 1)] = f - g;
					f = 0;
		...
		{
			l = i - 1;
			if(!aster_linear::equal<T>(d[i - 1], 0, epsilon))
			{
				for(j = 1; j <= l; j++)
				{
					g = 0;
		...
		for(l = 1; l <= D; l++)
		{
			...
			do
			{
				for(m = l; m <= D - 1; m++)
				{
					dd = aster_linear::abs(d[m - 1]) + aster_linear::abs(d[m - 1 + 1]);
					if(aster_linear::equal<T>(aster_linear::abs(e[m - 1]) + dd, dd, epsilon))
						break;
				}
				if(m != l)
				{
					...
					for(i = m - 1; i >= l; i--)
					{
						...
						if(aster_linear::equal<T>(r, 0, epsilon))
						{
							d[i - 1 + 1] -= p;
							e[m - 1] = 0;
							break;
					...
					if(aster_linear::equal<T>(r, 0, epsilon) && (i >= l))
						continue;
					d[l - 1] -= p;
					e[l - 1] = g;
					e[m - 1] = 0;
