...
namespace aster_linear
{
	...
	ASTER_LINEAR_FUNC_QUALIFIER bool intersectRayPlane
	(
	...
	{
		typename genType::value_type d = aster_linear::dot(dir, planeNormal);
		typename genType::value_type Epsilon = std::numeric_limits<typename genType::value_type>::epsilon();

		if(aster_linear::abs(d) > Epsilon)  // if dir and planeNormal are not perpendicular
		{
			typename genType::value_type const tmp_intersectionDistance = 	aster_linear::dot(planeOrig - orig, planeNormal) / d;
	...
	ASTER_LINEAR_FUNC_QUALIFIER bool intersectRayTriangle
	(
	...
	)
	{
		...
		vec<3, T, Q> const p = aster_linear::cross(dir, edge2);

		// if determinant is near zero, ray lies in plane of triangle
		T const det = aster_linear::dot(edge1, p);
		...
		if(det > std::numeric_limits<T>::epsilon())
		{
			...
			baryPosition.x = aster_linear::dot(dist, p);
			if(baryPosition.x < static_cast<T>(0) || baryPosition.x > det)
				return false;

			// prepare to test V parameter
			Perpendicular = aster_linear::cross(dist, edge1);

			// calculate V parameter and test bounds
			baryPosition.y = aster_linear::dot(dir, Perpendicular);
		...
		distance = aster_linear::dot(edge2, Perpendicular) * inv_det;
	...
	template<typename genType>
	ASTER_LINEAR_FUNC_QUALIFIER bool intersectLineTriangle
	(
		genType const& orig, genType const& dir,
	...
	template<typename genType>
	ASTER_LINEAR_FUNC_QUALIFIER bool intersectRaySphere
	(
	...
	template<typename genType>
	ASTER_LINEAR_FUNC_QUALIFIER bool intersectLineSphere
	(
