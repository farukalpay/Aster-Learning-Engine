...
size_t _OBJWriter_::getMaterial(AsterTopology::Vec3f _color) const
{
...
size_t _OBJWriter_::getMaterial(AsterTopology::Vec4f _color) const
{
...
writeMaterial(std::ostream& _out, BaseExporter& _be, Options _opt) const
{
  ...
  for (size_t i=0, nF=_be.n_faces(); i<nF; ++i)
  {
    ...
    if ( _opt.color_has_alpha() ){
      cA  = color_cast<AsterTopology::Vec4f> (_be.colorA( FaceHandle(int(i)) ));
      getMaterial(cA);
    }else{
    //and without alpha
      c  = color_cast<AsterTopology::Vec3f> (_be.color( FaceHandle(int(i)) ));
      getMaterial(c);
    }
  }
