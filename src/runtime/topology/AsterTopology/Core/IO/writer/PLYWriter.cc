...
void _PLYWriter_::write_customProp(std::ostream& _out, const CustomProperty& _prop, size_t _index) const
{
  if (_prop.type == ValueTypeCHAR)
    writeProxy(_prop.type,_out, castProperty<signed char>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeUCHAR || _prop.type == ValueTypeUINT8)
    writeProxy(_prop.type,_out, castProperty<unsigned char>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeSHORT)
    writeProxy(_prop.type,_out, castProperty<signed short>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeUSHORT)
    writeProxy(_prop.type,_out, castProperty<unsigned short>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeUINT)
    writeProxy(_prop.type,_out, castProperty<unsigned int>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeINT || _prop.type == ValueTypeINT32)
    writeProxy(_prop.type,_out, castProperty<signed int>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeFLOAT || _prop.type == ValueTypeFLOAT32)
    writeProxy(_prop.type,_out, castProperty<float>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
  else if (_prop.type == ValueTypeDOUBLE)
    writeProxy(_prop.type,_out, castProperty<double>(_prop.property)->data()[_index], AsterTopology::GenProg::Bool2Type<binary>());
}
...
write_ascii(std::ostream& _out, BaseExporter& _be, Options _opt) const
{
  ...
  Vec3f v, n;
  AsterTopology::Vec3ui c;
  AsterTopology::Vec4ui cA;
  AsterTopology::Vec3f cf;
  AsterTopology::Vec4f cAf;
  AsterTopology::Vec2f t;
  VertexHandle vh;
...
write_binary(std::ostream& _out, BaseExporter& _be, Options _opt) const
{
  ...
  Vec3f v, n;
  Vec2f t;
  AsterTopology::Vec4uc c;
  AsterTopology::Vec4f cf;
  VertexHandle vh;
  std::vector<VertexHandle> vhandles;
