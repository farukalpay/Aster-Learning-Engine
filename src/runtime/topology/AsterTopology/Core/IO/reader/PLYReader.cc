...
{
  typedef AsterTopology::VPropHandleT<T> PropT;
};

template<typename T>
struct Handle2Prop<T,FaceHandle>
{
  typedef AsterTopology::FPropHandleT<T> PropT;
};
...
void _PLYReader_::readCreateCustomProperty(std::istream& _in, BaseImporter& _bi, Handle _h, const std::string& _propName, const _PLYReader_::ValueType _valueType, const _PLYReader_::ValueType _listType) const
{
  if (_listType == Unsupported) //no list type defined -> property is not a list
  {
    ...
    //read and assign
    T in;
    read(_valueType, _in, in, AsterTopology::GenProg::Bool2Type<binary>());
  ...
  else
  {
    ...
    //init vector
    int numberOfValues;
    read(_listType, _in, numberOfValues, AsterTopology::GenProg::Bool2Type<binary>());
    std::vector<T> vec;
...
bool _PLYReader_::read_ascii(std::istream& _in, BaseImporter& _bi, const Options& _opt) const {
    ...
    unsigned int nV;
    AsterTopology::Vec3f v, n;
    std::string trash;
    AsterTopology::Vec2f t;
    AsterTopology::Vec4i c;
    float tmp;
				...
				for (size_t propertyIndex = 0; propertyIndex < e_it->properties_.size(); ++propertyIndex) {
					...
					case COLORRED:
						if (prop.value == ValueTypeFLOAT32 ||
							...
							_in >> tmp;
							c[0] = static_cast<AsterTopology::Vec4i::value_type> (tmp * 255.0f);
						}
						else
							_in >> c[0];
						break;
					case COLORGREEN:
						if (prop.value == ValueTypeFLOAT32 ||
							prop.value == ValueTypeFLOAT) {
							_in >> tmp;
							c[1] = static_cast<AsterTopology::Vec4i::value_type> (tmp * 255.0f);
						}
						else
							_in >> c[1];
						break;
					case COLORBLUE:
						if (prop.value == ValueTypeFLOAT32 ||
							prop.value == ValueTypeFLOAT) {
							_in >> tmp;
							c[2] = static_cast<AsterTopology::Vec4i::value_type> (tmp * 255.0f);
						}
						else
							_in >> c[2];
						break;
					case COLORALPHA:
						if (prop.value == ValueTypeFLOAT32 ||
							prop.value == ValueTypeFLOAT) {
							_in >> tmp;
							c[3] = static_cast<AsterTopology::Vec4i::value_type> (tmp * 255.0f);
						}
						else
							_in >> c[3];
						break;
...
bool _PLYReader_::read_binary(std::istream& _in, BaseImporter& _bi, bool /*_swap*/, const Options& _opt) const {
    ...
    AsterTopology::Vec3f        v, n;  // Vertex
    AsterTopology::Vec2f        t;  // TexCoords
    BaseImporter::VHandles vhandles;
    VertexHandle           vh;
    AsterTopology::Vec4i        c;  // Color
    float                  tmp;
