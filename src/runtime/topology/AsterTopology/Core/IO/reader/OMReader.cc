...
{
  // check whether importer can give us an AsterTopology BaseKernel
  if (!_bi.kernel())
    return false;
...
bool _OMReader_::read_binary(std::istream& _is, BaseImporter& _bi, Options& _opt) const
{
  ...
  if (header_.version_ > _OMWriter_::get_version())
  {
    omerr() << "File uses .om version " << OMFormat::as_string(header_.version_) << " but reader only "
            ...
            << "Please update your AsterTopology." << std::endl;
    return false;
  }
...
bool _OMReader_::read_binary_vertex_chunk(std::istream &_is, BaseImporter &_bi, Options &_opt, bool _swap) const
{
  ...
  AsterTopology::Vec3f v3f;
  AsterTopology::Vec2f v2f;
  AsterTopology::Vec3uc v3uc; // rgb
  AsterTopology::Attributes::StatusInfo status;
  ...
  switch (chunk_header_.type_) {
    case Chunk::Type_Pos:
      assert( OMFormat::dimensions(chunk_header_) == size_t(AsterTopology::Vec3f::dim()));
    ...
    case Chunk::Type_Texcoord:
      assert( OMFormat::dimensions(chunk_header_) == size_t(AsterTopology::Vec2f::dim()));
...
bool _OMReader_::read_binary_face_chunk(std::istream &_is, BaseImporter &_bi, Options &_opt, bool _swap) const
{
  ...
  size_t fidx = 0;
  AsterTopology::Vec3f v3f;  // normal
  AsterTopology::Vec3uc v3uc; // rgb
  AsterTopology::Attributes::StatusInfo status;
