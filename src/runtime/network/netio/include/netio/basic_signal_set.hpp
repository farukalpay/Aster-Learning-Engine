#ifndef NETIO_BASIC_SIGNAL_SET_HPP
#define NETIO_BASIC_SIGNAL_SET_HPP
...
#include "netio/detail/signal_set_service.hpp"
 ...
 * netio::signal_set signals(my_context, SIGINT, SIGTERM);
 *
 ...
 * using Netio. The application must not also register a signal handler using
...
public:
   ...
   * @code netio::signal_set signals(ex);
    ...
    : impl_(0, ex)
  {
    ...
    netio::detail::throw_error(ec, "add");
  }
   ...
   *
   * @note This constructor is equivalent to performing:
   * @code netio::signal_set signals(context);
   * signals.add(signal_number_1); @endcode
   */
  template <typename ExecutionContext>
   ...
   */
  NETIO_SYNC_OP_VOID add(int signal_number,
      netio::error_code& ec)
  {
    impl_.get_service().add(impl_.get_implementation(), signal_number, ec);
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  NETIO_SYNC_OP_VOID add(int signal_number, flags_t f,
      netio::error_code& ec)
  {
    impl_.get_service().add(impl_.get_implementation(), signal_number, f, ec);
    NETIO_SYNC_OP_VOID_RETURN(ec);
  ...
  void remove(int signal_number)
  {
    ...
    netio::detail::throw_error(ec, "remove");
  }
   ...
   * signal number.
   */
  NETIO_SYNC_OP_VOID remove(int signal_number,
      netio::error_code& ec)
  {
    impl_.get_service().remove(impl_.get_implementation(), signal_number, ec);
    NETIO_SYNC_OP_VOID_RETURN(ec);
  ...
  void clear()
  {
    ...
    netio::detail::throw_error(ec, "clear");
  }
   ...
   * @note Removes all queued notifications.
   */
  NETIO_SYNC_OP_VOID clear(netio::error_code& ec)
  {
    impl_.get_service().clear(impl_.get_implementation(), ec);
    NETIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * operation will be invoked with the netio::error::operation_aborted
   * error code.
   *
   ...
   * error code netio::error::operation_aborted.
   *
   ...
   * @code void(netio::error_code, int) @endcode
   *
   ...
   */
  template <
    NETIO_COMPLETION_TOKEN_FOR(void (netio::error_code, int))
      SignalToken = default_completion_token_t<executor_type>>
  auto async_wait(
      SignalToken&& token = default_completion_token_t<executor_type>())
    -> decltype(
      async_initiate<SignalToken, void (netio::error_code, int)>(
        declval<initiate_async_wait>(), token))
  {
    return async_initiate<SignalToken, void (netio::error_code, int)>(
...
private:
  ...
  public:
    ...
    void operator()(SignalHandler&& handler) const
    {
      ...
      NETIO_SIGNAL_HANDLER_CHECK(SignalHandler, handler) type_check;
...
#endif // NETIO_BASIC_SIGNAL_SET_HPP
