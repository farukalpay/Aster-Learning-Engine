#ifndef NETIO_DEFERRED_HPP
#define NETIO_DEFERRED_HPP
...
public:
  ...
  template <NETIO_COMPLETION_TOKEN_FOR(Signatures...) CompletionToken>
...
class NETIO_NODISCARD deferred_values
{
private:
...
public:
  ...
  template <NETIO_COMPLETION_TOKEN_FOR(void(Values...)) CompletionToken>
  auto operator()(CompletionToken&& token) &&
    -> decltype(
      this->invoke_helper(
...
class NETIO_NODISCARD deferred_async_operation
{
private:
...
public:
  ...
  template <NETIO_COMPLETION_TOKEN_FOR(Signature) CompletionToken>
  auto operator()(CompletionToken&& token) &&
    -> decltype(
      this->invoke_helper(
        static_cast<CompletionToken&&>(token),
...
template <typename Head, typename Tail>
class NETIO_NODISCARD deferred_sequence :
...
class NETIO_NODISCARD deferred_conditional
{
private:
 ...
 *
 * or by explicitly passing the special value @c netio::deferred:
 *
 * @code auto my_deferred_op
 *   = my_socket.async_read_some(my_buffer,
 *       netio::deferred); @endcode
 *
...
/**
 * See the documentation for netio::deferred_t for a usage example.
 */
NETIO_INLINE_VARIABLE constexpr deferred_t deferred;

} // namespace netio

#include "netio/detail/pop_options.hpp"

#include "netio/impl/deferred.hpp"

#endif // NETIO_DEFERRED_HPP
