#ifndef NETIO_PREFER_HPP
#define NETIO_PREFER_HPP
...
#include "netio/traits/prefer_free.hpp"
#include "netio/traits/prefer_member.hpp"
#include "netio/traits/require_free.hpp"
#include "netio/traits/require_member.hpp"
#include "netio/traits/static_require.hpp"
 ...
 * expression <tt>netio::prefer(E, P0, Pn...)</tt> for some subexpressions
 ...
 *   <tt>netio::prefer(E, P0, Pn...)</tt> is ill-formed.
 *
 ...
 *
 * @li Otherwise,
 *   <tt>netio::prefer(netio::prefer(E, P0), Pn...)</tt>
 *   if <tt>N > 0</tt> and the expression
 *   <tt>netio::prefer(netio::prefer(E, P0), Pn...)</tt>
 *   is a valid expression.
 *
 * @li Otherwise, <tt>netio::prefer(E, P0, Pn...)</tt> is ill-formed.
 */
 ...
 * @c true_type if the expression <tt>netio::prefer(std::declval<T>(),
 ...
 * type of the expression <tt>netio::prefer(std::declval<T>(),
...
namespace netio_prefer_fn {

using netio::conditional_t;
using netio::decay_t;
using netio::declval;
using netio::enable_if_t;
using netio::is_applicable_property;
using netio::traits::prefer_free;
using netio::traits::prefer_member;
using netio::traits::require_free;
using netio::traits::require_member;
using netio::traits::static_require;
...
struct impl
{
  ...
  && defined(NETIO_HAS_DEDUCED_PREFER_MEMBER_TRAIT)
    struct type
    {
      template <typename P>
      auto require(P&& p)
...
#else // defined(NETIO_HAS_DEDUCED_REQUIRE_MEMBER_TRAIT)
      //   && defined(NETIO_HAS_DEDUCED_PREFER_MEMBER_TRAIT)
    typedef T type;
  ...
  NETIO_NODISCARD constexpr enable_if_t<
...
static constexpr const netio_prefer_fn::impl&
  prefer = netio_prefer_fn::static_instance<>::instance;

} // namespace

typedef netio_prefer_fn::impl prefer_t;

template <typename T, typename... Properties>
struct can_prefer :
  integral_constant<bool,
    netio_prefer_fn::call_traits<
      prefer_t, T, void(Properties...)>::overload
        != netio_prefer_fn::ill_formed>
{
};
...
struct prefer_result
{
  typedef typename netio_prefer_fn::call_traits<
