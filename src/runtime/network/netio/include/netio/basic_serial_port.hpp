#ifndef NETIO_BASIC_SERIAL_PORT_HPP
#define NETIO_BASIC_SERIAL_PORT_HPP
...
public:
   ...
   */
  NETIO_SYNC_OP_VOID open(const std::string& device,
      netio::error_code& ec)
  {
    impl_.get_service().open(impl_.get_implementation(), device, ec);
   ...
   */
  NETIO_SYNC_OP_VOID assign(const native_handle_type& native_serial_port,
      netio::error_code& ec)
  {
    impl_.get_service().assign(impl_.get_implementation(),
        native_serial_port, ec);
  ...
  void send_break()
  {
    ...
    netio::detail::throw_error(ec, "send_break");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  NETIO_SYNC_OP_VOID send_break(netio::error_code& ec)
  {
    impl_.get_service().send_break(impl_.get_implementation(), ec);
    NETIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * netio::serial_port_base::baud_rate @n
   * netio::serial_port_base::flow_control @n
   * netio::serial_port_base::parity @n
   * netio::serial_port_base::stop_bits @n
   * netio::serial_port_base::character_size
   */
   ...
   */
  template <typename SettableSerialPortOption>
  NETIO_SYNC_OP_VOID set_option(const SettableSerialPortOption& option,
      netio::error_code& ec)
  {
    impl_.get_service().set_option(impl_.get_implementation(), option, ec);
    NETIO_SYNC_OP_VOID_RETURN(ec);
   ...
   */
  template <typename GettableSerialPortOption>
  NETIO_SYNC_OP_VOID get_option(GettableSerialPortOption& option,
   ...
   *
   * @throws netio::system_error Thrown on failure. An error code of
   * netio::error::eof indicates that the connection was closed by the
   * peer.
   *
   ...
   * @code
   * basic_serial_port.write_some(netio::buffer(data, size));
   * @endcode
   ...
   *     netio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * basic_serial_port.read_some(netio::buffer(data, size));
   * @endcode
...
#endif // NETIO_BASIC_SERIAL_PORT_HPP
