#ifndef NETIO_EXECUTION_PREFER_ONLY_HPP
#define NETIO_EXECUTION_PREFER_ONLY_HPP
...
struct prefer_only_property<InnerProperty,
    void_t<
      decltype(netio::declval<const InnerProperty>().value())
    >
  >
{
  InnerProperty property;
  ...
  constexpr auto value() const
    noexcept(noexcept(netio::declval<const InnerProperty>().value()))
    -> decltype(netio::declval<const InnerProperty>().value())
  {
  ...
  detail::prefer_only_property<InnerProperty>
{
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    noexcept(is_nothrow_prefer<const Executor&, const InnerProperty&>::value)
       ...
       //   && !defined(__clang__)
  {
    return netio::prefer(ex, p.property);
  }
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    noexcept(is_nothrow_query<const Executor&, const InnerProperty&>::value)
       ...
       //   && !defined(__clang__)
  {
    return netio::query(ex, p.property);
  }
};
...
#endif // NETIO_EXECUTION_PREFER_ONLY_HPP
