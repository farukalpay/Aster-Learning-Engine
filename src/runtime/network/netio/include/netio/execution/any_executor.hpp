#ifndef NETIO_EXECUTION_ANY_EXECUTOR_HPP
#define NETIO_EXECUTION_ANY_EXECUTOR_HPP
...
#include "netio/detail/cstddef.hpp"
#include "netio/detail/executor_function.hpp"
#include "netio/detail/memory.hpp"
#include "netio/detail/non_const_lvalue.hpp"
#include "netio/detail/scoped_ptr.hpp"
...
public:
   ...
   * size_t n = netio::query(ex, execution::occupancy); @endcode
   */
  template <typename Property>
  ...
  /**
   * Throws netio::bad_executor if the polymorphic wrapper has no target.
   */
...
public:
  ...
  {
    if (impl_)
      netio::detail::ref_count_up(impl_->ref_count_);
  }
  ...
  {
    if (impl_)
      if (netio::detail::ref_count_down(impl_->ref_count_))
        delete impl_;
  }
...
private:
  ...
  struct impl_base
  {
    ...
    virtual ~impl_base() {}
    virtual void* get() = 0;
    netio::detail::atomic_count ref_count_;
  };
...
public:
  ...
  void execute(F&& f) const
  {
    if (target_)
    {
      ...
      {
        netio::detail::non_const_lvalue<F> f2(f);
...
protected:
  ...
  typedef netio::detail::executor_function function;
  typedef netio::detail::executor_function_view function_view;
...
# pragma warning (disable:4702)
  ...
  static void query_fn_non_void(void*, const void* ex, const void* prop,
      enable_if_t<
        netio::can_query<const Ex&, const Prop&>::value
          && is_same<typename Prop::polymorphic_query_result_type, void>::value
      >*)
  {
    netio::query(*static_cast<const Ex*>(ex),
  ...
  static Poly require_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return netio::require(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
  ...
  static Poly prefer_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return netio::prefer(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
...
private:
  ...
  {
    return netio::query(ex, execution::blocking);
  }
...
//  template <typename...> friend class any_executor;
  ...
  typedef aligned_storage<
      sizeof(netio::detail::shared_ptr<void>) + sizeof(void*),
      alignment_of<netio::detail::shared_ptr<void>>::value
    >::type object_type;
...
public:
      ...
      >* = 0) const
  {
    ...
    prop_fns_[found::index].query(&result, object_fns_->target(*this),
        &static_cast<const typename found::type&>(p));
    return *netio::detail::scoped_ptr<
      typename found::query_result_type>(result);
  }
...
#if !defined(NETIO_HAS_DEDUCED_PREFER_FREE_TRAIT)
...
#endif // !defined(NETIO_HAS_DEDUCED_PREFER_FREE_TRAIT)
...
#endif // NETIO_EXECUTION_ANY_EXECUTOR_HPP
