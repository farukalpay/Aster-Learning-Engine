#ifndef NETIO_EXECUTION_BLOCKING_ADAPTATION_HPP
#define NETIO_EXECUTION_BLOCKING_ADAPTATION_HPP
...
struct blocking_adaptation_t
{
  ...
  {
    return netio::query(ex, disallowed_t());
  }
...
#endif // !defined(__clang__)
  {
    return netio::query(ex, allowed_t());
  }

  NETIO_STATIC_CONSTEXPR_DEFAULT_INIT(disallowed_t, disallowed);
  NETIO_STATIC_CONSTEXPR_DEFAULT_INIT(allowed_t, allowed);
...
public:
  ...
  struct cleanup
  {
    ~cleanup()
    {
      netio::detail::mutex::scoped_lock lock(state_->mutex_);
...
NETIO_INLINE_VARIABLE constexpr blocking_adaptation_t blocking_adaptation;
...
#endif // NETIO_EXECUTION_BLOCKING_ADAPTATION_HPP
