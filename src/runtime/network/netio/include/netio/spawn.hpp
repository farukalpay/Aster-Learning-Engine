#ifndef NETIO_SPAWN_HPP
#define NETIO_SPAWN_HPP
...
#include "netio/detail/exception.hpp"
...
public:
   ...
   * } @endcode
   */
  basic_yield_context operator[](netio::error_code& ec) const
  {
 ...
 *
 * @code netio::spawn(my_strand, do_echo, netio::detached);
 *
 * // ...
 *
 * void do_echo(netio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           netio::buffer(data), yield);
 *
 *       netio::async_write(my_socket,
 *           netio::buffer(data, length), yield);
 *     }
 *   }
...
#include "netio/impl/spawn.hpp"

#endif // NETIO_SPAWN_HPP
