#ifndef NETIO_BASIC_STREAM_FILE_HPP
#define NETIO_BASIC_STREAM_FILE_HPP
...
#if !defined(NETIO_BASIC_STREAM_FILE_FWD_DECL)
#define NETIO_BASIC_STREAM_FILE_FWD_DECL

// Forward declaration with defaulted arguments.
template <typename Executor = any_io_executor>
class basic_stream_file;

#endif // !defined(NETIO_BASIC_STREAM_FILE_FWD_DECL)
...
public:
  ...
  uint64_t seek(int64_t offset, file_base::seek_basis whence)
  {
    ...
    netio::detail::throw_error(ec, "seek");
    return n;
  }
   ...
   * netio::error::eof indicates that the end of the file was reached.
   *
   ...
   * @code
   * file.write_some(netio::buffer(data, size));
   * @endcode
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_write_some(netio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * file.read_some(netio::buffer(data, size));
   * @endcode
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_read_some(netio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
...
#endif // NETIO_BASIC_STREAM_FILE_HPP
