#ifndef NETIO_IMPL_CONFIG_IPP
#define NETIO_IMPL_CONFIG_IPP
...
public:
      ...
      char* value, std::size_t value_len) const override
  {
    if (std::strcmp(section, "scheduler") == 0)
    {
      ...
      {
        if (NETIO_CONCURRENCY_HINT_IS_SPECIAL(concurrency_hint_))
        {
          return
            !NETIO_CONCURRENCY_HINT_IS_LOCKING(
      ...
      {
        return NETIO_CONCURRENCY_HINT_IS_LOCKING(
...
config_from_concurrency_hint::config_from_concurrency_hint()
  : concurrency_hint_(NETIO_CONCURRENCY_HINT_DEFAULT)
{
}
...
void config_from_concurrency_hint::make(execution_context& ctx) const
{
  (void)make_service<detail::config_from_concurrency_hint_service>(ctx,
      concurrency_hint_ == 1
        ? NETIO_CONCURRENCY_HINT_1 : concurrency_hint_);
}
