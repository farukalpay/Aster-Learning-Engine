#ifndef NETIO_IMPL_WRITE_AT_HPP
#define NETIO_IMPL_WRITE_AT_HPP
...
#include "netio/detail/base_from_completion_cond.hpp"
#include "netio/detail/bind_handler.hpp"
#include "netio/detail/consuming_buffers.hpp"
#include "netio/detail/dependent_type.hpp"
...
namespace detail
{
  ...
  {
    ec = netio::error_code();
    netio::detail::consuming_buffers<const_buffer,
    ...
    uint64_t offset, const ConstBufferSequence& buffers)
{
  ...
  netio::detail::throw_error(ec, "write_at");
...
template <typename SyncRandomAccessWriteDevice, typename Allocator,
    typename CompletionCondition>
std::size_t write_at(SyncRandomAccessWriteDevice& d,
    uint64_t offset, netio::basic_streambuf<Allocator>& b,
    CompletionCondition completion_condition, netio::error_code& ec,
    constraint_t<
      is_completion_condition<CompletionCondition>::value
    >)
  ...
  public:
        ...
        std::size_t bytes_transferred, int start = 0)
    {
      ...
      switch (start_ = start)
      {
        ...
        {
          {
            NETIO_HANDLER_LOCATION((__FILE__, __LINE__, "async_write_at"));
        ...
        static_cast<WriteHandler&&>(handler_)(
            static_cast<const netio::error_code&>(ec),
            static_cast<const std::size_t&>(buffers_.total_consumed()));
      }
    }

  //private:
    typedef netio::detail::consuming_buffers<const_buffer,
...
namespace detail
{
  ...
  public:
    write_at_streambuf_op(
        netio::basic_streambuf<Allocator>& streambuf,
        WriteHandler& handler)
    ...
    void operator()(const netio::error_code& ec,
  ...
  //private:
    netio::basic_streambuf<Allocator>& streambuf_;
    WriteHandler handler_;
  };
