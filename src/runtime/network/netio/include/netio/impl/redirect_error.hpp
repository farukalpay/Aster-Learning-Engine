#ifndef NETIO_IMPL_REDIRECT_ERROR_HPP
#define NETIO_IMPL_REDIRECT_ERROR_HPP
...
public:
  ...
  redirect_error_handler(netio::error_code& ec,
      RedirectedHandler&& h)
    : ec_(ec),
  ...
  enable_if_t<
    !is_same<decay_t<Arg>, netio::error_code>::value
  >
  ...
  void operator()(const netio::error_code& ec, Args&&... args)
  {
    ec_ = ec;
...
struct redirect_error_signature<R(netio::error_code, Args...)>
{
  typedef R type(Args...);
};

template <typename R, typename... Args>
struct redirect_error_signature<R(const netio::error_code&, Args...)>
{
  typedef R type(Args...);
};
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(netio::error_code, Args...) noexcept>
{
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(const netio::error_code&, Args...) noexcept>
{
  typedef R type(Args...) & noexcept;
};
      ...
      typename detail::redirect_error_signature<Signature>::type>
{
  ...
  struct init_wrapper : detail::initiation_base<Initiation>
  {
    ...
    template <typename Handler, typename... Args>
    void operator()(Handler&& handler,
        netio::error_code* ec, Args&&... args) const &
    {
...
#endif // NETIO_IMPL_REDIRECT_ERROR_HPP
