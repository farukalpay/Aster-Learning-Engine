#ifndef NETIO_IMPL_SPAWN_HPP
#define NETIO_IMPL_SPAWN_HPP
  ...
  void suspend_with(void (*fn)(void*), void* arg)
  {
    if (throw_if_cancelled_)
      if (!!cancellation_state_.cancelled())
        throw_error(netio::error::operation_aborted, "yield");
...
template <typename Executor, typename R>
class spawn_handler<Executor, R(netio::error_code)>
  : public spawn_handler_base<Executor>
{
public:
  typedef void return_type;
  typedef netio::error_code* result_type;
...
public:
  ...
  {
    if (*result != no_error)
      netio::throw_exception(static_cast<Disposition&&>(*result));
  }
...
template <typename Executor, typename R, typename T>
class spawn_handler<Executor, R(netio::error_code, T)>
  : public spawn_handler_base<Executor>
{
public:
  typedef T return_type;
  ...
  void operator()(netio::error_code ec, T value)
  {
    if (this->yield_.ec_)
    {
      *this->yield_.ec_ = ec;
...
class spawn_handler<Executor, R(netio::error_code, Ts...)>
  : public spawn_handler_base<Executor>
{
public:
...
inline bool netio_handler_is_continuation(spawn_handler<Executor, Signature>*)
{
  return true;
}
...
#else // defined(NETIO_HAS_VARIADIC_LAMBDA_CAPTURES)
    ...
    enable_if_t<
      is_same<
        typename associated_executor<Handler,
          Executor>::netio_associated_executor_is_unspecialised,
        void
      >::value
    >>
{
...
template <typename Executor, typename F,
    NETIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<
