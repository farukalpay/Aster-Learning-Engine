#ifndef NETIO_IMPL_BUFFERED_WRITE_STREAM_HPP
#define NETIO_IMPL_BUFFERED_WRITE_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(netio::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const netio::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = netio::buffer_copy(
...
#endif // NETIO_IMPL_BUFFERED_WRITE_STREAM_HPP
