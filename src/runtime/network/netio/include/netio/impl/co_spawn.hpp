#ifndef NETIO_IMPL_CO_SPAWN_HPP
#define NETIO_IMPL_CO_SPAWN_HPP
...
struct co_spawn_state
{
  ...
  co_spawn_state(H&& h, const Executor& ex, F&& f)
    : handler(std::forward<H>(h)),
      spawn_work(ex),
      handler_work(netio::get_associated_executor(handler, ex)),
...
struct co_spawn_dispatch
{
  ...
  auto operator()(CompletionToken&& token) const
    -> decltype(netio::dispatch(std::forward<CompletionToken>(token)))
  {
    return netio::dispatch(std::forward<CompletionToken>(token));
  }
};
...
struct co_spawn_post
{
  template <typename CompletionToken>
  auto operator()(CompletionToken&& token) const
    -> decltype(netio::post(std::forward<CompletionToken>(token)))
  {
    return netio::post(std::forward<CompletionToken>(token));
  }
};
...
public:
  ...
  void operator()(cancellation_type_t type)
  {
    ...
    netio::dispatch(ex_, [sig, type]{ sig->emit(type); });
  }
...
public:
  ...
  void operator()(Handler&& handler, F&& f) const
  {
    ...
    auto slot = netio::get_associated_cancellation_slot(handler);
...
inline NETIO_INITFN_AUTO_RESULT_TYPE(CompletionToken,
