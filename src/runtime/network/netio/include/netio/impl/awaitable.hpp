#ifndef NETIO_IMPL_AWAITABLE_HPP
#define NETIO_IMPL_AWAITABLE_HPP
...
{
public:
  NETIO_DECL void launch(void (*pump_fn)(void*), void* arg);
  NETIO_DECL bool is_launching();
};
...
{
public:
#if !defined(NETIO_DISABLE_AWAITABLE_FRAME_RECYCLING)
  void* operator new(std::size_t size)
  {
    return netio::detail::thread_info_base::allocate(
        netio::detail::thread_info_base::awaitable_frame_tag(),
        netio::detail::thread_context::top_of_thread_call_stack(),
        size);
  }

  void operator delete(void* pointer, std::size_t size)
  {
    netio::detail::thread_info_base::deallocate(
        netio::detail::thread_info_base::awaitable_frame_tag(),
        netio::detail::thread_context::top_of_thread_call_stack(),
        pointer, size);
  }
#endif // !defined(NETIO_DISABLE_AWAITABLE_FRAME_RECYCLING)
...
private:
  ...
  netio::cancellation_slot parent_cancellation_slot_;
  netio::cancellation_state cancellation_state_;
  bool has_executor_;
...
public:
  ...
  static void resume(result_type& result)
  {
    if (*result != no_error)
    {
      ...
      netio::throw_exception(static_cast<Disposition&&>(d));
    }
  }
...
#if defined(NETIO_ENABLE_HANDLER_TRACKING)
# if defined(NETIO_HAS_SOURCE_LOCATION)
          NETIO_HANDLER_LOCATION((self->location_.file_name(),
...
struct coroutine_traits<netio::awaitable<T, Executor>, Args...>
{
  typedef netio::detail::awaitable_frame<T, Executor> promise_type;
};
...
#endif // NETIO_IMPL_AWAITABLE_HPP
