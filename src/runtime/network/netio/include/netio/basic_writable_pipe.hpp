#ifndef NETIO_BASIC_WRITABLE_PIPE_HPP
#define NETIO_BASIC_WRITABLE_PIPE_HPP
...
# include "netio/detail/win_iocp_handle_service.hpp"
...
public:
   ...
   */
  NETIO_SYNC_OP_VOID assign(const native_handle_type& native_pipe,
      netio::error_code& ec)
  {
    impl_.get_service().assign(impl_.get_implementation(), native_pipe, ec);
   ...
   * @code
   * pipe.write_some(netio::buffer(data, size));
   * @endcode
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * pipe.async_write_some(netio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
...
#endif // NETIO_BASIC_WRITABLE_PIPE_HPP
