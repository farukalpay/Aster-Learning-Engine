#ifndef NETIO_DISPOSITION_HPP
#define NETIO_DISPOSITION_HPP
 ...
 * treated as a disposition by netio.
 *
...
template <typename T>
NETIO_CONCEPT disposition = is_disposition<T>::value;

#define NETIO_DISPOSITION ::netio::disposition

#else // defined(NETIO_HAS_CONCEPTS)

#define NETIO_DISPOSITION typename

#endif // defined(NETIO_HAS_CONCEPTS)

/// Specialisation of @c disposition_traits for @c error_code.
template <>
struct disposition_traits<netio::error_code>
{
  static bool not_an_error(const netio::error_code& ec) noexcept
  {
    return !ec;
  }

  static void throw_exception(const netio::error_code& ec)
  {
    detail::throw_exception(netio::system_error(ec));
  }

  static std::exception_ptr to_exception_ptr(
      const netio::error_code& ec) noexcept
  {
    return ec
      ? std::make_exception_ptr(netio::system_error(ec))
      : nullptr;
  }
};
...
NETIO_INLINE_VARIABLE constexpr no_error_t no_error;
