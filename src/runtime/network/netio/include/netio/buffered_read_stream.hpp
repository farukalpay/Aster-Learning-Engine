#ifndef NETIO_BUFFERED_READ_STREAM_HPP
#define NETIO_BUFFERED_READ_STREAM_HPP
...
#include "netio/detail/buffer_resize_guard.hpp"
#include "netio/detail/buffered_stream_storage.hpp"
...
public:
...
#else
  NETIO_STATIC_CONSTANT(std::size_t, default_buffer_size = 1024);
#endif
  ...
  std::size_t fill(netio::error_code& ec);
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(netio::error_code& ec)
  {
    ec = netio::error_code();
    return storage_.size();
  }
...
private:
  ...
  {
    std::size_t bytes_copied = netio::buffer_copy(
  ...
  {
    return netio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
...
#include "netio/impl/buffered_read_stream.hpp"

#endif // NETIO_BUFFERED_READ_STREAM_HPP
