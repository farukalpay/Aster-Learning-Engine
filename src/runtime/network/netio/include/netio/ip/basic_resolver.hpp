#ifndef NETIO_IP_BASIC_RESOLVER_HPP
#define NETIO_IP_BASIC_RESOLVER_HPP
...
# include "netio/detail/winrt_resolver_service.hpp"
#else
# include "netio/detail/resolver_service.hpp"
#endif
...
public:
   ...
   * will be invoked with the netio::error::operation_aborted error code.
   */
  void cancel()
  {
   ...
   */
  results_type resolve(NETIO_STRING_VIEW_PARAM host,
      NETIO_STRING_VIEW_PARAM service)
  {
   ...
   */
  results_type resolve(NETIO_STRING_VIEW_PARAM host,
      NETIO_STRING_VIEW_PARAM service, netio::error_code& ec)
  {
   ...
   */
  results_type resolve(NETIO_STRING_VIEW_PARAM host,
      NETIO_STRING_VIEW_PARAM service, resolver_base::flags resolve_flags)
  {
    netio::error_code ec;
    ...
    netio::detail::throw_error(ec, "resolve");
    return r;
  }
  ...
  results_type resolve(const protocol_type& protocol,
      NETIO_STRING_VIEW_PARAM host, NETIO_STRING_VIEW_PARAM service)
  {
  ...
  results_type resolve(const protocol_type& protocol,
      NETIO_STRING_VIEW_PARAM host, NETIO_STRING_VIEW_PARAM service,
      resolver_base::flags resolve_flags, netio::error_code& ec)
  {
    basic_resolver_query<protocol_type> q(
        protocol, static_cast<std::string>(host),
        static_cast<std::string>(service), resolve_flags);
   ...
   * @code void(netio::error_code, results_type) @endcode
   *
   * @note On POSIX systems, host names may be locally defined in the file
   * <tt>/etc/hosts</tt>. On Windows, host names may be defined in the file
   * <tt>c:\\windows\\system32\\drivers\\etc\\hosts</tt>. Remote host name
  ...
  auto async_resolve(NETIO_STRING_VIEW_PARAM host,
      NETIO_STRING_VIEW_PARAM service,
      ResolveToken&& token = default_completion_token_t<executor_type>())
    -> decltype(
      netio::async_initiate<ResolveToken,
        void (netio::error_code, results_type)>(
          ...
          declval<basic_resolver_query<protocol_type>&>()))
  {
    ...
    return netio::async_initiate<ResolveToken,
      void (netio::error_code, results_type)>(
   ...
   * function is guaranteed to return a non-empty range.
   */
  results_type resolve(const endpoint_type& e, netio::error_code& ec)
  {
...
private:
  ...
  public:
        ...
        const Query& q) const
    {
      ...
      NETIO_RESOLVE_HANDLER_CHECK(
          ResolveHandler, handler, results_type) type_check;

      netio::detail::non_const_lvalue<ResolveHandler> handler2(handler);
...
# if defined(NETIO_WINDOWS_RUNTIME)
  netio::detail::io_object_impl<
    netio::detail::winrt_resolver_service<InternetProtocol>,
    Executor> impl_;
# else
  netio::detail::io_object_impl<
    netio::detail::resolver_service<InternetProtocol>,
    Executor> impl_;
# endif
};
...
#endif // NETIO_IP_BASIC_RESOLVER_HPP
