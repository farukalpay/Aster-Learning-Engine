#ifndef NETIO_IP_BASIC_ENDPOINT_HPP
#define NETIO_IP_BASIC_ENDPOINT_HPP
...
/**
 * The netio::ip::basic_endpoint class template describes an endpoint that
...
public:
...
#else
  typedef netio::detail::socket_addr_type data_type;
#endif
   ...
   * @code
   * netio::ip::tcp::endpoint ep(netio::ip::tcp::v4(), 1234);
   * @endcode
   *
   * To specify an IPv6 UDP endpoint for port 9876, use:
   * @code
   * netio::ip::udp::endpoint ep(netio::ip::udp::v6(), 9876);
   * @endcode
   */
  ...
  basic_endpoint(const netio::ip::address& addr,
      port_type port_num) noexcept
    : impl_(addr, port_num)
  {
  }
  ...
  netio::ip::address address() const noexcept
  {
    return impl_.address();
  }

  /// Set the IP address associated with the endpoint.
  void address(const netio::ip::address& addr) noexcept
  {
    impl_.address(addr);
  }
...
private:
  ...
  netio::ip::detail::endpoint impl_;
};
 ...
 *
 * @relates netio::ip::basic_endpoint
 */
...
template <typename InternetProtocol>
struct hash<netio::ip::basic_endpoint<InternetProtocol>>
{
  std::size_t operator()(
      const netio::ip::basic_endpoint<InternetProtocol>& ep)
    const noexcept
  {
    std::size_t hash1 = std::hash<netio::ip::address>()(ep.address());
...
#include "netio/ip/impl/basic_endpoint.hpp"

#endif // NETIO_IP_BASIC_ENDPOINT_HPP
