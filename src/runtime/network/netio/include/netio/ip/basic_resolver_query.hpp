#ifndef NETIO_IP_BASIC_RESOLVER_QUERY_HPP
#define NETIO_IP_BASIC_RESOLVER_QUERY_HPP
...
/**
 * The netio::ip::basic_resolver_query class template describes a query
...
public:
      ...
      service_name_(service)
  {
    typename InternetProtocol::endpoint endpoint;
    hints_.ai_flags = static_cast<int>(resolve_flags);
    hints_.ai_family = NETIO_OS_DEF(AF_UNSPEC);
    hints_.ai_socktype = endpoint.protocol().type();
    hints_.ai_protocol = endpoint.protocol().protocol();
    hints_.ai_addrlen = 0;
    hints_.ai_canonname = 0;
  ...
  const netio::detail::addrinfo_type& hints() const
  {
    return hints_;
  }
...
private:
  netio::detail::addrinfo_type hints_;
...
#endif // NETIO_IP_BASIC_RESOLVER_QUERY_HPP
