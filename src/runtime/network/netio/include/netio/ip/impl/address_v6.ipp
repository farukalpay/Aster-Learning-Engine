#ifndef NETIO_IP_IMPL_ADDRESS_V6_IPP
#define NETIO_IP_IMPL_ADDRESS_V6_IPP
...
{
  netio::error_code ec;
  char addr_str[netio::detail::max_addr_v6_str_len];
  const char* addr =
    netio::detail::socket_ops::inet_ntop(
        NETIO_OS_DEF(AF_INET6), &addr_, addr_str,
        netio::detail::max_addr_v6_str_len, scope_id_, ec);
  if (addr == 0)
...
bool operator==(const address_v6& a1, const address_v6& a2) noexcept
{
  ...
  return memcmp(&a1.addr_, &a2.addr_,
      sizeof(netio::detail::in6_addr_type)) == 0
...
bool operator<(const address_v6& a1, const address_v6& a2) noexcept
{
  ...
  int memcmp_result = memcmp(&a1.addr_, &a2.addr_,
      sizeof(netio::detail::in6_addr_type));
  if (memcmp_result < 0)
    return true;
  if (memcmp_result > 0)
    return false;
    ...
    netio::error_code& ec) noexcept
{
  ...
  if (netio::detail::socket_ops::inet_pton(
        NETIO_OS_DEF(AF_INET6), str, &bytes[0], &scope_id, ec) <= 0)
    return address_v6();
...
#endif // NETIO_IP_IMPL_ADDRESS_V6_IPP
