#ifndef NETIO_IP_IMPL_ADDRESS_IPP
#define NETIO_IP_IMPL_ADDRESS_IPP
    ...
    const netio::ip::address_v4& ipv4_address) noexcept
{
  type_ = ipv4;
  ipv4_address_ = ipv4_address;
  ipv6_address_ = netio::ip::address_v6();
  return *this;
}
...
{
  type_ = ipv6;
  ipv4_address_ = netio::ip::address_v4();
  ipv6_address_ = ipv6_address;
  return *this;
}
...
{
  netio::ip::address_v6 ipv6_address =
    netio::ip::make_address_v6(str, ec);
  if (!ec)
    return address(ipv6_address);

  netio::ip::address_v4 ipv4_address =
    netio::ip::make_address_v4(str, ec);
  if (!ec)
...
netio::ip::address_v4 address::to_v4() const
{
  if (type_ != ipv4)
  {
    bad_address_cast ex;
...
netio::ip::address_v6 address::to_v6() const
{
  if (type_ != ipv6)
  {
    bad_address_cast ex;
...
#endif // NETIO_IP_IMPL_ADDRESS_IPP
