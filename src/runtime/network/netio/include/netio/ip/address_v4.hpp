#ifndef NETIO_IP_ADDRESS_V4_HPP
#define NETIO_IP_ADDRESS_V4_HPP
...
/**
 * The netio::ip::address_v4 class provides the ability to use and
...
public:
...
#else
  typedef netio::detail::array<unsigned char, 4> bytes_type;
#endif
   ...
   */
  NETIO_DECL explicit address_v4(const bytes_type& bytes);

  /// Construct an address from an unsigned integer in host byte order.
  /**
   * Initialises the @c address_v4 object such that <tt>to_uint() == addr</tt>.
   */
  NETIO_DECL explicit address_v4(uint_type addr);
  ...
  NETIO_DECL uint_type to_uint() const noexcept;
...
private:
  ...
  netio::detail::in4_addr_type addr_;
};
 ...
 * @relates address_v4
 */
NETIO_DECL address_v4 make_address_v4(const char* str);
 ...
 * @relates address_v4
 */
NETIO_DECL address_v4 make_address_v4(const std::string& str);
 ...
 * @relates address_v4
 */
NETIO_DECL address_v4 make_address_v4(string_view str);
...
template <>
struct hash<netio::ip::address_v4>
{
  std::size_t operator()(const netio::ip::address_v4& addr)
    const noexcept
  {
