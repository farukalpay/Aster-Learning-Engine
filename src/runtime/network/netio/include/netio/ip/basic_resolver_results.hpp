#ifndef NETIO_IP_BASIC_RESOLVER_RESULTS_HPP
#define NETIO_IP_BASIC_RESOLVER_RESULTS_HPP
...
/**
 * The netio::ip::basic_resolver_results class template is used to define
...
public:
  ...
  static basic_resolver_results create(
      netio::detail::addrinfo_type* address_info,
      const std::string& host_name, const std::string& service_name)
  {
    ...
    while (address_info)
    {
      if (address_info->ai_family == NETIO_OS_DEF(AF_INET)
          || address_info->ai_family == NETIO_OS_DEF(AF_INET6))
      {
        const std::size_t expected_size =
          address_info->ai_family == NETIO_OS_DEF(AF_INET)
            ? sizeof(netio::detail::sockaddr_in4_type)
            : sizeof(netio::detail::sockaddr_in6_type);
...
# if defined(NETIO_WINDOWS_RUNTIME)
  ...
  static basic_resolver_results create(
      ...
      const netio::detail::addrinfo_type& hints,
      const std::string& host_name, const std::string& service_name)
  {
    basic_resolver_results results;
    if (endpoints->Size)
    {
      ...
      for (unsigned int i = 0; i < endpoints->Size; ++i)
      {
        ...
        if (hints.ai_family == NETIO_OS_DEF(AF_INET)
        ...
        results.values_->push_back(
            basic_resolver_entry<InternetProtocol>(
              typename InternetProtocol::endpoint(
                ip::make_address(
                  netio::detail::winrt_utils::string(
                    pair->RemoteHostName->CanonicalName)),
                netio::detail::winrt_utils::integer(
...
#endif // NETIO_IP_BASIC_RESOLVER_RESULTS_HPP
