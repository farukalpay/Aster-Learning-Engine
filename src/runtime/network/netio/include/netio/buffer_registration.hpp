#ifndef NETIO_BUFFER_REGISTRATION_HPP
#define NETIO_BUFFER_REGISTRATION_HPP
...
public:
  ...
  buffer_registration(const Executor& ex,
    ...
    : buffer_sequence_(buffer_sequence),
      buffers_(
          NETIO_REBIND_ALLOC(allocator_type,
            mutable_registered_buffer)(alloc))
  {
    init_buffers(buffer_registration::get_context(ex),
        netio::buffer_sequence_begin(buffer_sequence_),
        netio::buffer_sequence_end(buffer_sequence_));
  }
...
private:
  ...
  void init_buffers(execution_context& ctx, Iterator begin, Iterator end)
  {
    ...
    std::vector<iovec,
      NETIO_REBIND_ALLOC(allocator_type, iovec)> iovecs(n,
          NETIO_REBIND_ALLOC(allocator_type, iovec)(
...
#endif // NETIO_BUFFER_REGISTRATION_HPP
