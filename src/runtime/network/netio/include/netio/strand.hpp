#ifndef NETIO_STRAND_HPP
#define NETIO_STRAND_HPP
...
#include "netio/detail/strand_executor_service.hpp"
...
public:
   ...
   *
   * For example:
   * @code netio::strand<my_executor_type> ex = ...;
   ...
   *
   * For example:
   * @code netio::strand<my_executor_type> ex1 = ...;
    ...
    noexcept(is_nothrow_require<const Executor&, Property>::value)
  {
    return strand<decay_t<require_result_t<const Executor&, Property>>>(
        netio::require(executor_, p), impl_);
  }
    ...
    noexcept(is_nothrow_prefer<const Executor&, Property>::value)
  {
    return strand<decay_t<prefer_result_t<const Executor&, Property>>>(
        netio::prefer(executor_, p), impl_);
  }
...
private:
      ...
      > = 0)
  {
    return use_service<detail::strand_executor_service>(
        netio::query(ex, execution::context)).create_implementation();
  }
  ...
  {
    return netio::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type, const Property& property) const
  {
    execution::blocking_t result = netio::query(executor_, property);
...
/** @defgroup make_strand netio::make_strand
 *
 * @brief The netio::make_strand function creates a @ref strand object for
...
# if defined(NETIO_IO_CONTEXT_HPP)
#  include "netio/io_context_strand.hpp"
# endif // defined(NETIO_IO_CONTEXT_HPP)
#endif // !defined(NETIO_NO_EXTENSIONS)

#endif // NETIO_STRAND_HPP
