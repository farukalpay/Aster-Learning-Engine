#ifndef NETIO_BUFFERS_ITERATOR_HPP
#define NETIO_BUFFERS_ITERATOR_HPP
...
public:
...
#endif // defined(__GNUC__) && (__GNUC__ == 4) && (__GNUC_MINOR__ == 3)
  {
    ...
    new_iter.begin_ = netio::buffer_sequence_begin(buffers);
    new_iter.current_ = netio::buffer_sequence_begin(buffers);
    new_iter.end_ = netio::buffer_sequence_end(buffers);
...
private:
  ...
  void increment()
  {
    NETIO_ASSERT(current_ != end_ && "iterator out of bounds");
    ++position_;
  ...
  void decrement()
  {
    NETIO_ASSERT(position_ > 0 && "iterator out of bounds");
    --position_;
  ...
  void advance(std::ptrdiff_t n)
  {
    if (n > 0)
    {
      ...
      for (;;)
      {
        ...
        if (++current_ == end_)
        {
          NETIO_ASSERT(n == 0 && "iterator out of bounds");
    ...
    {
      std::size_t abs_n = -n;
      NETIO_ASSERT(position_ >= abs_n && "iterator out of bounds");
      for (;;)
      {
        // Check if the advance can be satisfied by the current buffer.
        ...
        if (current_ == begin_)
        {
          NETIO_ASSERT(abs_n == 0 && "iterator out of bounds");
          current_buffer_position_ = 0;
          return;
        }
...
#endif // NETIO_BUFFERS_ITERATOR_HPP
