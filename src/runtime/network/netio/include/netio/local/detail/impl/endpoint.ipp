#ifndef NETIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
#define NETIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
...
{
  if (new_size > sizeof(netio::detail::sockaddr_un_type))
  {
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    netio::error_code ec(netio::error::name_too_long);
    netio::detail::throw_error(ec);
  }

  using namespace std; // For memset and memcpy.
  memset(&data_.local, 0, sizeof(netio::detail::sockaddr_un_type));
...
#endif // NETIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
