#ifndef NETIO_DETAIL_CONSUMING_BUFFERS_HPP
#define NETIO_DETAIL_CONSUMING_BUFFERS_HPP
...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = netio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = netio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = netio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return netio::buffer(buffer_ + total_consumed_, max_size);
  }
...
public:
  ...
  boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    ...
    result[0] = netio::buffer(result[0] + total_consumed_, max_size);
    result[1] = netio::buffer(
...
#endif // NETIO_DETAIL_CONSUMING_BUFFERS_HPP
