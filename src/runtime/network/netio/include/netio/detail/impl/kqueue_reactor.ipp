#ifndef NETIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define NETIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define NETIO_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
...
kqueue_reactor::kqueue_reactor(netio::execution_context& ctx)
        ...
        io_locking_, io_locking_spin_count_)
{
  ...
  NETIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
      EVFILT_READ, EV_ADD, 0, 0, &interrupter_);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
  {
    netio::error_code error(errno,
        netio::error::get_system_category());
    netio::detail::throw_error(error);
  }
}
...
{
  if (fork_ev == netio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      netio::error_code ec(errno,
          netio::error::get_system_category());
      netio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        NETIO_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        NETIO_KQUEUE_EV_SET(&events[1], state->descriptor_,
            EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, state);
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          netio::error_code ec(errno,
              netio::error::get_system_category());
          netio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  NETIO_HANDLER_REACTOR_REGISTRATION((
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  NETIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        NETIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        NETIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  for (int i = 0; i < max_ops; ++i)
  {
    ...
    {
      op->ec_ = netio::error::operation_aborted;
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      NETIO_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      NETIO_KQUEUE_EV_SET(&events[1], descriptor,
    ...
    NETIO_HANDLER_REACTOR_DEREGISTRATION((
          context(), static_cast<uintmax_t>(descriptor),
          reinterpret_cast<uintmax_t>(descriptor_data)));
...
void kqueue_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    if (ptr != &interrupter_)
    {
      ...
      {
      case EVFILT_READ:
        event_mask |= NETIO_HANDLER_REACTOR_READ_EVENT;
        break;
      case EVFILT_WRITE:
        event_mask |= NETIO_HANDLER_REACTOR_WRITE_EVENT;
        break;
      }
      if ((events[i].flags & (EV_ERROR | EV_OOBAND)) != 0)
        event_mask |= NETIO_HANDLER_REACTOR_ERROR_EVENT;
      NETIO_HANDLER_REACTOR_EVENTS((context(),
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        NETIO_KQUEUE_EV_SET(&delete_events[0],
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    netio::error_code ec(errno,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
...
#undef NETIO_KQUEUE_EV_SET

#include "netio/detail/pop_options.hpp"

#endif // defined(NETIO_HAS_KQUEUE)

#endif // NETIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
