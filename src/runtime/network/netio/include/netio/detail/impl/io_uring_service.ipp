#ifndef NETIO_DETAIL_IMPL_IO_URING_SERVICE_IPP
#define NETIO_DETAIL_IMPL_IO_URING_SERVICE_IPP
...
io_uring_service::io_uring_service(netio::execution_context& ctx)
    ...
    netio::execution_context::fork_event fork_ev)
{
  switch (fork_ev)
  {
  case netio::execution_context::fork_prepare:
    {
  ...
  case netio::execution_context::fork_parent:
    // Restart the timeout and eventfd operations.
    update_timeout();
    register_with_reactor();
    break;

  case netio::execution_context::fork_child:
    {
    ...
    int op_type, io_uring_operation* op)
{
  ...
  {
    netio::error_code ec(ENOBUFS,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "io_uring_get_sqe");
  }
}
...
void io_uring_service::register_buffers(const ::iovec* v, unsigned n)
{
  ...
  if (result < 0)
  {
    netio::error_code ec(-result,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "io_uring_register_buffers");
  }
}
...
void io_uring_service::init_ring()
{
  ...
  if (result < 0)
  {
    ...
    netio::error_code ec(-result,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "io_uring_queue_init");
  }
  ...
  if (event_fd_ < 0)
  {
    netio::error_code ec(-result,
        netio::error::get_system_category());
    ::io_uring_queue_exit(&ring_);
    netio::detail::throw_error(ec, "eventfd");
  }
...
operation* io_uring_service::io_queue::perform_io(int result)
{
  ...
  if (result != -ECANCELED || cancel_requested_)
  {
    if (io_uring_operation* op = op_queue_.front())
    {
      if (result < 0)
      {
        op->ec_.assign(-result, netio::error::get_system_category());
  ...
  if (!op_queue_.empty())
  {
    ...
    else
    {
      ...
      while (io_uring_operation* op = op_queue_.front())
      {
        op->ec_ = netio::error::no_buffer_space;
        op_queue_.pop();
        io_cleanup.ops_.push(op);
      }
    }
...
#endif // NETIO_DETAIL_IMPL_IO_URING_SERVICE_IPP
