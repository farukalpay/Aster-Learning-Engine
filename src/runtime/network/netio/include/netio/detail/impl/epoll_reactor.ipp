#ifndef NETIO_DETAIL_IMPL_EPOLL_REACTOR_IPP
#define NETIO_DETAIL_IMPL_EPOLL_REACTOR_IPP
...
epoll_reactor::epoll_reactor(netio::execution_context& ctx)
        ...
        state != 0; state = state->next_)
    {
      if (state->registered_events_ != 0)
      {
        ...
        if (result != 0)
        {
          ...
          netio::detail::throw_error(ec, "epoll re-registration");
        }
      }
    }
  }
...
#else // defined(NETIO_HAS_TIMERFD)
...
int epoll_reactor::do_epoll_create()
{
  ...
  if (fd == -1)
  {
    netio::error_code ec(errno,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "epoll");
  }
...
#endif // NETIO_DETAIL_IMPL_EPOLL_REACTOR_IPP
