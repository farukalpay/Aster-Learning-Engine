#ifndef NETIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
#define NETIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
...
win_iocp_io_context::win_iocp_io_context(
    netio::execution_context& ctx, bool own_thread)
    ...
    concurrency_hint_(config(ctx).get("scheduler", "concurrency_hint", -1))
{
  ...
  if (!iocp_.handle)
  {
    ...
    netio::detail::throw_error(ec, "iocp");
  }
...
win_iocp_io_context::win_iocp_io_context(
    win_iocp_io_context::internal, netio::execution_context& ctx)
  : execution_context_service_base<win_iocp_io_context>(ctx),
    iocp_(),
    outstanding_work_(0),
    stopped_(0),
...
netio::error_code win_iocp_io_context::register_handle(
...
size_t win_iocp_io_context::run(netio::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(netio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, netio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(netio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(netio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        netio::error_code ec(last_error,
            netio::error::get_system_category());
        netio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const netio::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
    ...
    win_iocp_thread_info& this_thread, netio::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      netio::error_code result_ec(last_error,
          netio::error::get_system_category());
      ...
      {
        result_ec = netio::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<netio::error_category*>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      netio::error_code ec(last_error,
          netio::error::get_system_category());
      netio::detail::throw_error(ec, "timer");
    }
...
#endif // NETIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
