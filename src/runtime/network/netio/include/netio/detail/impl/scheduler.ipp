#ifndef NETIO_DETAIL_IMPL_SCHEDULER_IPP
#define NETIO_DETAIL_IMPL_SCHEDULER_IPP
...
#include "netio/detail/scheduler_thread_info.hpp"
...
scheduler::scheduler(netio::execution_context& ctx,
    bool own_thread, get_task_func_type get_task)
  : netio::detail::execution_context_service_base<scheduler>(ctx),
...
scheduler::scheduler(scheduler::internal, netio::execution_context& ctx)
  : netio::detail::execution_context_service_base<scheduler>(ctx),
    one_thread_(false),
    mutex_(true, 0),
    task_(0),
...
std::size_t scheduler::run(netio::error_code& ec)
{
  ec = netio::error_code();
...
std::size_t scheduler::run_one(netio::error_code& ec)
{
  ec = netio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, netio::error_code& ec)
{
  ec = netio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(netio::error_code& ec)
{
  ec = netio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(netio::error_code& ec)
{
  ec = netio::error_code();
  if (outstanding_work_ == 0)
  {
...
void scheduler::compensating_work_started()
{
  ...
  NETIO_ASSUME(this_thread != 0); // Only called from inside scheduler.
...
scheduler_task* scheduler::get_default_task(netio::execution_context& ctx)
{
#if defined(NETIO_HAS_IO_URING_AS_DEFAULT)
...
#endif // NETIO_DETAIL_IMPL_SCHEDULER_IPP
