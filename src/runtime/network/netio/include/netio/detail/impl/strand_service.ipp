#ifndef NETIO_DETAIL_IMPL_STRAND_SERVICE_IPP
#define NETIO_DETAIL_IMPL_STRAND_SERVICE_IPP
...
strand_service::strand_service(netio::io_context& io_context)
  : netio::detail::service_base<strand_service>(io_context),
    io_context_(io_context),
    io_context_impl_(netio::use_service<io_context_impl>(io_context)),
    mutex_(),
    salt_(0)
{
}
...
#if defined(NETIO_ENABLE_SEQUENTIAL_STRAND_ALLOCATION)
  std::size_t index = salt;
#else // defined(NETIO_ENABLE_SEQUENTIAL_STRAND_ALLOCATION)
  std::size_t index = reinterpret_cast<std::size_t>(&impl);
  index += (reinterpret_cast<std::size_t>(&impl) >> 3);
  index ^= salt + 0x9e3779b9 + (index << 6) + (index >> 2);
#endif // defined(NETIO_ENABLE_SEQUENTIAL_STRAND_ALLOCATION)
...
void strand_service::do_dispatch(implementation_type& impl, operation* op)
{
  ...
  if (can_dispatch && !impl->locked_)
  {
    ...
    op->complete(&io_context_impl_, netio::error_code(), 0);
    return;
  }
...
#endif // NETIO_DETAIL_IMPL_STRAND_SERVICE_IPP
