#ifndef NETIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
#define NETIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
...
#include "netio/detail/win_iocp_socket_service_base.hpp"
    ...
    win_iocp_socket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    netio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
netio::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
    netio::error_code& ec)
{
  if (is_open(impl))
  {
    NETIO_HANDLER_OPERATION((iocp_service_.context(),
...
netio::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    netio::error_code& ec)
{
  if (!is_open(impl))
...
#else // defined(NETIO_ENABLE_CANCELIO)
  else
  {
...
netio::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, netio::error_code& ec)
{
  if (is_open(impl))
  {
...
netio::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, netio::error_code& ec)
{
  if (is_open(impl))
  {
    ec = netio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, netio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, netio::error::already_open);
  else
  {
    netio::error_code ec;
    ...
    long* cancel_requested, operation* op)
{
  ...
  if (cancel_requested)
    if (::InterlockedExchangeAdd(cancel_requested, 0) == 1)
      iocp_service_.on_completion(op, netio::error::operation_aborted);
    ...
    int op_type, reactor_op* op)
{
  ...
  if (is_open(impl))
  {
    ...
    op->ec_ = netio::error::bad_descriptor;
    ...
    win_iocp_socket_connect_op_base* op, operation* iocp_op)
{
      ...
      || family == NETIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      socket_ops::bind(impl.socket_, &a.base,
          family == NETIO_OS_DEF(AF_INET)
          ? sizeof(a.v4) : sizeof(a.v6), op->ec_);
      if (op->ec_ && op->ec_ != netio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return -1;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == netio::error::in_progress
          || op->ec_ == netio::error::would_block)
      {
        op->ec_ = netio::error_code();
...
{
#if defined(NETIO_DISABLE_CONNECTEX)
  (void)impl;
  (void)type;
  return 0;
#else // defined(NETIO_DISABLE_CONNECTEX)
  if (type != NETIO_OS_DEF(SOCK_STREAM)
      && type != NETIO_OS_DEF(SOCK_SEQPACKET))
    return 0;
...
#endif // defined(NETIO_DISABLE_CONNECTEX)
}
...
#endif // NETIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
