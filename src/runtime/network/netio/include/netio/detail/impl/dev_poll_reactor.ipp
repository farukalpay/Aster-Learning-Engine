#ifndef NETIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
#define NETIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
...
dev_poll_reactor::dev_poll_reactor(netio::execution_context& ctx)
  : netio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
{
  netio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, netio::error::operation_aborted);
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, netio::error::operation_aborted);
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      netio::error_code ec = netio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    netio::error_code ec(errno,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
...
#endif // NETIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
