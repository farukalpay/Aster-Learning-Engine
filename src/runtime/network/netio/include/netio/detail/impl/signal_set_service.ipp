#ifndef NETIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
#define NETIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
...
#include <cstring>
#include <stdexcept>
#include "netio/detail/signal_blocker.hpp"
...
signal_state* get_signal_state()
{
  static signal_state state = {
    NETIO_STATIC_MUTEX_INIT, -1, -1, false, 0,
    { 0 }, { signal_set_base::flags_t() } };
  return &state;
}

void netio_signal_handler(int signal_number)
{
...
#if defined(NETIO_HAS_SIGNAL) && !defined(NETIO_HAS_SIGACTION)
  ::signal(signal_number, netio_signal_handler);
#endif // defined(NETIO_HAS_SIGNAL) && !defined(NETIO_HAS_SIGACTION)
}
...
# if defined(NETIO_HAS_IO_URING_AS_DEFAULT)
  pipe_read_op()
    : io_uring_operation(netio::error_code(), &pipe_read_op::do_prepare,
...
signal_set_service::signal_set_service(execution_context& context)
  : execution_context_service_base<signal_set_service>(context),
    scheduler_(netio::use_service<scheduler_impl>(context)),
#if !defined(NETIO_WINDOWS) \
  && !defined(NETIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
# if defined(NETIO_HAS_IO_URING_AS_DEFAULT)
...
# else // defined(NETIO_HAS_IO_URING_AS_DEFAULT)
    reactor_(netio::use_service<reactor>(context)),
# endif // defined(NETIO_HAS_IO_URING_AS_DEFAULT)
#endif // !defined(NETIO_WINDOWS)
       //   && !defined(NETIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
# endif // defined(NETIO_HAS_IO_URING_AS_DEFAULT)
  ...
  case execution_context::fork_child:
    if (state->fork_prepared_)
    {
      netio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
netio::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl, int signal_number,
    signal_set_base::flags_t f, netio::error_code& ec)
{
...
# if defined(NETIO_HAS_SIGACTION)
      ...
      sa.sa_handler = netio_signal_handler;
      sigfillset(&sa.sa_mask);
      if (f != signal_set_base::flags::dont_care)
        sa.sa_flags = static_cast<int>(f);
      if (::sigaction(signal_number, &sa, 0) == -1)
# else // defined(NETIO_HAS_SIGACTION)
      if (::signal(signal_number, netio_signal_handler) == SIG_ERR)
# endif // defined(NETIO_HAS_SIGACTION)
      {
...
netio::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
...
netio::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
    netio::error_code& ec)
{
  signal_state* state = get_signal_state();
...
netio::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    netio::error_code& ec)
{
  NETIO_HANDLER_OPERATION((scheduler_.context(),
...
void signal_set_service::open_descriptors()
{
  ...
  else
  {
    netio::error_code ec(errno,
        netio::error::get_system_category());
    netio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(NETIO_WINDOWS)
       //   && !defined(NETIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
#endif // NETIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
