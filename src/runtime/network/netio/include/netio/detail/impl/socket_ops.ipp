#ifndef NETIO_DETAIL_SOCKET_OPS_IPP
#define NETIO_DETAIL_SOCKET_OPS_IPP
    ...
    netio::error_code& ec, bool is_error_condition)
{
  ...
  {
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
    ec = netio::error_code(WSAGetLastError(),
    ...
    void* addr, std::size_t* addrlen, netio::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    else if (ec == netio::error::connection_aborted)
    {
    ...
    socket_type new_socket, netio::error_code& ec)
{
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = netio::error::connection_aborted;
...
int shutdown(socket_type s, int what, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
    ...
    std::size_t addrlen, netio::error_code& ec)
{
  ...
  if (result != 0 && ec == netio::error::try_again)
  {
    if (static_cast<const socket_addr_type*>(addr)->sa_family == AF_UNIX)
      ec = netio::error::in_progress;
    else
      ec = netio::error::no_buffer_space;
  }
    ...
    std::size_t addrlen, netio::error_code& ec)
{
  ...
  ec = netio::error_code(connect_error,
      netio::error::get_system_category());
}

#if defined(NETIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, netio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  case ERROR_CONNECTION_REFUSED:
    ec = netio::error::connection_refused;
    break;
  case ERROR_NETWORK_UNREACHABLE:
    ec = netio::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = netio::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = netio::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, netio::error_code& ec)
{
...
bool sockatmark(socket_type s, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
...
size_t available(socket_type s, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
    ...
    int flags, netio::error_code& ec)
{
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = netio::error::connection_reset;
...
size_t sync_recv1(socket_type s, state_type state, void* data,
    size_t size, int flags, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
    ...
    netio::error_code& ec, size_t bytes_transferred)
{
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
    if (cancel_token.expired())
      ec = netio::error::operation_aborted;
    else
      ec = netio::error::connection_reset;
  }
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
...
signed_size_type recvfrom(socket_type s, buf* bufs, size_t count,
    int flags, void* addr, std::size_t* addrlen, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
...
int getpeername(socket_type s, void* addr, std::size_t* addrlen,
    bool cached, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
...
#if defined(NETIO_WINDOWS) && !defined(NETIO_WINDOWS_APP) \
  || defined(__CYGWIN__)
  if (cached)
  {
...
#else // defined(NETIO_WINDOWS) && !defined(NETIO_WINDOWS_APP)
      // || defined(__CYGWIN__)
  (void)cached;
#endif // defined(NETIO_WINDOWS) && !defined(NETIO_WINDOWS_APP)
       // || defined(__CYGWIN__)
...
int poll_connect(socket_type s, int msec, netio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = netio::error::bad_descriptor;
    ...
    unsigned long scope_id, netio::error_code& ec)
{
  ...
  if (af == NETIO_OS_DEF(AF_INET))
  {
    sprintf_s(dest, length, "%u.%u.%u.%u",
        bytes[0], bytes[1], bytes[2], bytes[3]);
    return dest;
  }
  else if (af == NETIO_OS_DEF(AF_INET6))
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length && b < 16)
    {
  ...
  {
    ec = netio::error::address_family_not_supported;
    return 0;
  }
#elif defined(NETIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.

  if (af != NETIO_OS_DEF(AF_INET) && af != NETIO_OS_DEF(AF_INET6))
  {
    ec = netio::error::address_family_not_supported;
    return 0;
  }
  ...
  if (af == NETIO_OS_DEF(AF_INET))
  {
    ...
    address.v4.sin_family = NETIO_OS_DEF(AF_INET);
  ...
  else // AF_INET6
  {
    ...
    address.v6.sin6_family = NETIO_OS_DEF(AF_INET6);
  ...
  if (result == 0 && !ec)
    ec = netio::error::invalid_argument;
  if (result != 0 && af == NETIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
...
#elif defined(NETIO_HAS_SECURE_RTL)
    ...
    unsigned long* scope_id, netio::error_code& ec)
{
  ...
  const bool is_v6 = (af == NETIO_OS_DEF(AF_INET6));
...
int gethostname(char* name, int namelen, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS_RUNTIME)
  try
  {
...
#else // defined(NETIO_WINDOWS_RUNTIME)
...
inline netio::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return netio::error::host_not_found;
  case TRY_AGAIN:
    return netio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return netio::error::no_recovery;
  case NO_DATA:
    return netio::error::no_data;
  default:
    NETIO_ASSERT(false);
    return netio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
    ...
    const addrinfo_type* hints, gai_search (&search)[2])
{
  ...
  if (host == 0 || host[0] == '\0')
  {
    if (hints->ai_flags & AI_PASSIVE)
    {
      ...
      {
      case NETIO_OS_DEF(AF_INET):
        search[search_count].host = "0.0.0.0";
        search[search_count].family = NETIO_OS_DEF(AF_INET);
        ++search_count;
        break;
      case NETIO_OS_DEF(AF_INET6):
        search[search_count].host = "0::0";
        search[search_count].family = NETIO_OS_DEF(AF_INET6);
        ++search_count;
        break;
      case NETIO_OS_DEF(AF_UNSPEC):
        search[search_count].host = "0::0";
        search[search_count].family = NETIO_OS_DEF(AF_INET6);
        ++search_count;
        search[search_count].host = "0.0.0.0";
  ...
  case NETIO_OS_DEF(AF_INET):
    {
      ...
      if (sinptr == 0)
        return EAI_MEMORY;
      sinptr->sin_family = NETIO_OS_DEF(AF_INET);
  ...
  case NETIO_OS_DEF(AF_INET6):
    {
      ...
      if (sin6ptr == 0)
        return EAI_MEMORY;
      sin6ptr->sin6_family = NETIO_OS_DEF(AF_INET6);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  hints.ai_family = NETIO_OS_DEF(AF_UNSPEC);
  if (hintsp)
    hints = *hintsp;
  ...
  if (hints.ai_family != NETIO_OS_DEF(AF_INET6))
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    netio::error_code ec;
    if (socket_ops::inet_pton(NETIO_OS_DEF(AF_INET),
          sptr->host, &inaddr, 0, ec) == 1)
    {
      if (hints.ai_family != NETIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != NETIO_OS_DEF(AF_INET))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == NETIO_OS_DEF(AF_INET))
      {
        rc = gai_aistruct(&ainext, &hints, &inaddr, NETIO_OS_DEF(AF_INET));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (socket_ops::inet_pton(NETIO_OS_DEF(AF_INET6),
          sptr->host, &in6addr, 0, ec) == 1)
    {
      if (hints.ai_family != NETIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != NETIO_OS_DEF(AF_INET6))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == NETIO_OS_DEF(AF_INET6))
      {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == netio::error::host_not_found)
        return EAI_NONAME;
      if (ec == netio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == netio::error::no_recovery)
        return EAI_FAIL;
      if (ec == netio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline netio::error_code getnameinfo_emulation(
    ...
    netio::error_code& ec)
{
  ...
  case NETIO_OS_DEF(AF_INET):
    ...
    {
      return ec = netio::error::invalid_argument;
    }
  ...
  case NETIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = netio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = netio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = netio::error::no_buffer_space;
      }
#if defined(NETIO_HAS_SNPRINTF)
...
inline netio::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return netio::error::no_recovery;
  case EAI_FAMILY:
    return netio::error::address_family_not_supported;
  case EAI_MEMORY:
    return netio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return netio::error::host_not_found;
  case EAI_SERVICE:
    return netio::error::service_not_found;
  case EAI_SOCKTYPE:
    return netio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
    return netio::error_code(
        WSAGetLastError(), netio::error::get_system_category());
#else
    return netio::error_code(
        errno, netio::error::get_system_category());
#endif
  }
}

netio::error_code getaddrinfo(const char* host,
...
# else
  ...
  return ec = translate_addrinfo_error(error);
# endif
#elif !defined(NETIO_HAS_GETADDRINFO)
  int error = getaddrinfo_emulation(host, service, &hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, &hints, result);
...
netio::error_code background_getaddrinfo(
...
netio::error_code getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, netio::error_code& ec)
{
#if defined(NETIO_WINDOWS) || defined(__CYGWIN__)
# if defined(NETIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
netio::error_code sync_getnameinfo(const void* addr,
...
netio::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const void* addr, std::size_t addrlen,
...
#endif // NETIO_DETAIL_SOCKET_OPS_IPP
