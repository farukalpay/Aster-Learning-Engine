#ifndef NETIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
#define NETIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
...
{
public:
  explicit overlapped_wrapper(netio::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
...
win_iocp_handle_service::win_iocp_handle_service(execution_context& context)
  : execution_context_service_base<win_iocp_handle_service>(context),
    iocp_service_(netio::use_service<win_iocp_io_context>(context)),
    nt_set_info_(0),
    mutex_(),
    impl_list_(0)
{
...
netio::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
...
netio::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
    netio::error_code& ec)
{
  if (is_open(impl))
  {
    NETIO_HANDLER_OPERATION((iocp_service_.context(), "handle",
...
netio::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    netio::error_code& ec)
{
  if (!is_open(impl))
...
size_t win_iocp_handle_service::do_write(
    ...
    const netio::const_buffer& buffer, netio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = netio::error::bad_descriptor;
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const netio::const_buffer& buffer, operation* op)
{
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const netio::mutable_buffer& buffer, netio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = netio::error::bad_descriptor;
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const netio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
...
#endif // NETIO_DETAIL_IMPL_WIN_IOCP_HANDLE_SERVICE_IPP
