#ifndef NETIO_DETAIL_SCHEDULER_HPP
#define NETIO_DETAIL_SCHEDULER_HPP
...
#include "netio/detail/conditionally_enabled_event.hpp"
...
public:
  ...
  typedef scheduler_task* (*get_task_func_type)(
      netio::execution_context&);

  // Constructor.
  NETIO_DECL scheduler(netio::execution_context& ctx,
      bool own_thread = true,
      get_task_func_type get_task = &scheduler::get_default_task);

  // Construct as an internal scheduler.
  NETIO_DECL scheduler(internal, netio::execution_context& ctx);

  // Destructor.
  NETIO_DECL ~scheduler();
  ...
  NETIO_DECL std::size_t run(netio::error_code& ec);

  // Run until interrupted or one operation is performed.
  NETIO_DECL std::size_t run_one(netio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  NETIO_DECL std::size_t wait_one(
      long usec, netio::error_code& ec);

  // Poll for operations without blocking.
  NETIO_DECL std::size_t poll(netio::error_code& ec);

  // Poll for one operation without blocking.
  NETIO_DECL std::size_t poll_one(netio::error_code& ec);
  ...
  NETIO_DECL void compensating_work_started();
  ...
  NETIO_DECL bool can_dispatch();

  /// Capture the current exception so it can be rethrown from a run function.
  NETIO_DECL void capture_current_exception();

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() has not yet been called for the operation.
  NETIO_DECL void post_immediate_completion(
      operation* op, bool is_continuation);

  // Request invocation of the given operations and return immediately. Assumes
  // that work_started() has not yet been called for the operations.
  NETIO_DECL void post_immediate_completions(std::size_t n,
      op_queue<operation>& ops, bool is_continuation);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operation.
  NETIO_DECL void post_deferred_completion(operation* op);

  // Request invocation of the given operations and return immediately. Assumes
  // that work_started() was previously called for each operation.
  NETIO_DECL void post_deferred_completions(op_queue<operation>& ops);

  // Enqueue the given operation following a failed attempt to dispatch the
  // operation for immediate invocation.
  NETIO_DECL void do_dispatch(operation* op);
  ...
  NETIO_DECL void abandon_operations(op_queue<operation>& ops);
...
private:
  ...
  NETIO_DECL std::size_t do_run_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const netio::error_code& ec);

  // Run at most one operation with a timeout. May block.
  NETIO_DECL std::size_t do_wait_one(mutex::scoped_lock& lock,
      thread_info& this_thread, long usec, const netio::error_code& ec);

  // Poll for at most one operation.
  NETIO_DECL std::size_t do_poll_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const netio::error_code& ec);

  // Stop the task and all idle threads.
  NETIO_DECL void stop_all_threads(mutex::scoped_lock& lock);

  // Wake a single idle thread, or the task, and always unlock the mutex.
  NETIO_DECL void wake_one_thread_and_unlock(
      mutex::scoped_lock& lock);

  // Get the default task.
  NETIO_DECL static scheduler_task* get_default_task(
      netio::execution_context& ctx);
...
#endif // NETIO_DETAIL_SCHEDULER_HPP
