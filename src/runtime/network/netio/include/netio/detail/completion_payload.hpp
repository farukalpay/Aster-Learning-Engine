#ifndef NETIO_DETAIL_COMPLETION_PAYLOAD_HPP
#define NETIO_DETAIL_COMPLETION_PAYLOAD_HPP
...
# include <variant>
#else // defined(NETIO_HAS_STD_VARIANT)
# include <new>
...
class completion_payload<R1(), R2(netio::error_code)>
{
public:
  typedef completion_message<R1()> void_message_type;
  typedef completion_message<R2(netio::error_code)> error_message_type;

  completion_payload(void_message_type&&)
    : message_(0, netio::error_code()),
      empty_(true)
  {
  }
...
#endif // NETIO_DETAIL_COMPLETION_PAYLOAD_HPP
