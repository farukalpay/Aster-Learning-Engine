#ifndef NETIO_DETAIL_STRAND_EXECUTOR_SERVICE_HPP
#define NETIO_DETAIL_STRAND_EXECUTOR_SERVICE_HPP
...
#include "netio/detail/executor_op.hpp"
...
public:
  ...
  NETIO_DECL explicit strand_executor_service(execution_context& context);

  // Destroy all user-defined handler objects owned by the service.
  NETIO_DECL void shutdown();

  // Create a new strand_executor implementation.
  NETIO_DECL implementation_type create_implementation();
  ...
  NETIO_DECL static bool running_in_this_thread(
...
private:
  ...
  NETIO_DECL static bool enqueue(const implementation_type& impl,
      scheduler_operation* op);

  // Transfers waiting handlers to the ready queue. Returns true if one or more
  // handlers were transferred.
  NETIO_DECL static bool push_waiting_to_ready(implementation_type& impl);

  // Invokes all ready-to-run handlers.
  NETIO_DECL static void run_ready_handlers(implementation_type& impl);
