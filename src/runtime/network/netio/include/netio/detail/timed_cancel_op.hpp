#ifndef NETIO_DETAIL_TIMED_CANCEL_OP_HPP
#define NETIO_DETAIL_TIMED_CANCEL_OP_HPP
...
public:
  ...
  void release()
  {
    if (--ref_count_ == 0)
    {
      ptr p = { netio::detail::addressof(handler_), this, this };
      Handler handler(static_cast<Handler&&>(handler_));
      p.h = netio::detail::addressof(handler);
      p.reset();
    }
  }
  ...
  void complete()
  {
    if (--ref_count_ == 0)
    {
      ...
      p.h = netio::detail::addressof(handler.handler());
      p.reset();
      handler();
    }
  }
...
public:
  ...
  void operator()(const netio::error_code&)
  {
    Op* op = op_;
    op_ = nullptr;
    op->handle_timer();
...
#endif // NETIO_DETAIL_TIMED_CANCEL_OP_HPP
