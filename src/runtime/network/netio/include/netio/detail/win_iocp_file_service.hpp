#ifndef NETIO_DETAIL_WIN_IOCP_FILE_SERVICE_HPP
#define NETIO_DETAIL_WIN_IOCP_FILE_SERVICE_HPP
...
#if defined(NETIO_HAS_IOCP) && defined(NETIO_HAS_FILE)
...
public:
  ...
  // Constructor.
  NETIO_DECL win_iocp_file_service(execution_context& context);
  ...
  NETIO_DECL uint64_t size(const implementation_type& impl,
      netio::error_code& ec) const;

  // Alter the size of the file.
  NETIO_DECL netio::error_code resize(implementation_type& impl,
      uint64_t n, netio::error_code& ec);

  // Synchronise the file to disk.
  NETIO_DECL netio::error_code sync_all(implementation_type& impl,
      netio::error_code& ec);

  // Synchronise the file data to disk.
  NETIO_DECL netio::error_code sync_data(implementation_type& impl,
      netio::error_code& ec);

  // Seek to a position in the file.
  NETIO_DECL uint64_t seek(implementation_type& impl, int64_t offset,
      ...
      const ConstBufferSequence& buffers, netio::error_code& ec)
  {
    ...
    impl.offset_ += netio::buffer_size(buffers);
...
#endif // defined(NETIO_HAS_IOCP) && defined(NETIO_HAS_FILE)

#endif // NETIO_DETAIL_WIN_IOCP_FILE_SERVICE_HPP
