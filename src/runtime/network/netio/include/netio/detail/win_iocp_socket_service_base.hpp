#ifndef NETIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_BASE_HPP
#define NETIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_BASE_HPP
...
#include "netio/detail/win_iocp_null_buffers_op.hpp"
#include "netio/detail/win_iocp_socket_connect_op.hpp"
#include "netio/detail/win_iocp_socket_send_op.hpp"
#include "netio/detail/win_iocp_socket_recv_op.hpp"
#include "netio/detail/win_iocp_socket_recvmsg_op.hpp"
#include "netio/detail/win_iocp_wait_op.hpp"
...
public:
...
#if defined(NETIO_ENABLE_CANCELIO)
    // The ID of the thread from which it is safe to cancel asynchronous
    // operations. 0 means no asynchronous operations have been started yet.
    // ~0 means asynchronous operations have been started from more than one
    // thread, and cancellation is not supported for the socket.
    DWORD safe_cancellation_thread_id_;
#endif // defined(NETIO_ENABLE_CANCELIO)
  ...
  // Constructor.
  NETIO_DECL win_iocp_socket_service_base(execution_context& context);
      ...
      socket_base::wait_type w, Handler& handler, const IoExecutor& io_ex)
  {
    ...
    NETIO_HANDLER_CREATION((context_, *p.p, "socket",
  ...
  NETIO_DECL void restart_accept_op(socket_type s,
...
protected:
  ...
  NETIO_DECL netio::error_code do_open(
      ...
      int protocol, netio::error_code& ec);

  // Assign a native socket to a socket implementation.
  NETIO_DECL netio::error_code do_assign(
      base_implementation_type& impl, int type,
      socket_type native_socket, netio::error_code& ec);
  ...
  NETIO_DECL void start_send_to_op(base_implementation_type& impl,
  ...
  NETIO_DECL int start_null_buffers_receive_op(
  ...
  NETIO_DECL void start_receive_from_op(base_implementation_type& impl,
      WSABUF* buffers, std::size_t buffer_count, void* addr,
      socket_base::message_flags flags, int* addrlen, operation* op);

  // Helper function to start an asynchronous accept operation.
  NETIO_DECL void start_accept_op(base_implementation_type& impl,
      bool peer_is_open, socket_holder& new_socket, int family, int type,
      int protocol, void* output_buffer, DWORD address_length, operation* op);

  // Start an asynchronous read or write operation using the reactor.
  NETIO_DECL void start_reactor_op(base_implementation_type& impl,
      int op_type, reactor_op* op);

  // Start the asynchronous connect operation using the reactor.
  NETIO_DECL int start_connect_op(base_implementation_type& impl,
  ...
  // destroyed.
  NETIO_DECL void close_for_destruction(base_implementation_type& impl);

  // Update the ID of the thread from which cancellation is safe.
  NETIO_DECL void update_cancellation_thread_id(
  ...
  NETIO_DECL select_reactor& get_reactor();
  ...
  NETIO_DECL connect_ex_fn get_connect_ex(
      base_implementation_type& impl, int type);
  ...
  NETIO_DECL nt_set_info_fn get_nt_set_info();

  // Helper function to emulate InterlockedCompareExchangePointer functionality
  // for:
  // - very old Platform SDKs; and
  // - platform SDKs where MSVC's /Wp64 option causes spurious warnings.
  NETIO_DECL void* interlocked_compare_exchange_pointer(
      void** dest, void* exch, void* cmp);

  // Helper function to emulate InterlockedExchangePointer functionality for:
  // - very old Platform SDKs; and
  // - platform SDKs where MSVC's /Wp64 option causes spurious warnings.
  NETIO_DECL void* interlocked_exchange_pointer(void** dest, void* val);
...
#endif // NETIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_BASE_HPP
