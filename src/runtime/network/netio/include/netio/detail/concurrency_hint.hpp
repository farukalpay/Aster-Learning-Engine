#ifndef NETIO_DETAIL_CONCURRENCY_HINT_HPP
#define NETIO_DETAIL_CONCURRENCY_HINT_HPP
...
#define NETIO_CONCURRENCY_HINT_ID 0xA5100000u
#define NETIO_CONCURRENCY_HINT_ID_MASK 0xFFFF0000u

// If set, this bit indicates that the scheduler should perform locking.
#define NETIO_CONCURRENCY_HINT_LOCKING_SCHEDULER 0x1u

// If set, this bit indicates that the reactor should perform locking when
// managing descriptor registrations.
#define NETIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION 0x2u

// If set, this bit indicates that the reactor should perform locking for I/O.
#define NETIO_CONCURRENCY_HINT_LOCKING_REACTOR_IO 0x4u

// Helper macro to determine if we have a special concurrency hint.
#define NETIO_CONCURRENCY_HINT_IS_SPECIAL(hint) \
  ((static_cast<unsigned>(hint) \
    & NETIO_CONCURRENCY_HINT_ID_MASK) \
      == NETIO_CONCURRENCY_HINT_ID)

// Helper macro to determine if locking is enabled for a given facility.
#define NETIO_CONCURRENCY_HINT_IS_LOCKING(facility, hint) \
  (((static_cast<unsigned>(hint) \
    & (NETIO_CONCURRENCY_HINT_ID_MASK \
      | NETIO_CONCURRENCY_HINT_LOCKING_ ## facility)) \
        ^ NETIO_CONCURRENCY_HINT_ID) != 0)
...
#define NETIO_CONCURRENCY_HINT_UNSAFE \
  static_cast<int>(NETIO_CONCURRENCY_HINT_ID)
...
#define NETIO_CONCURRENCY_HINT_UNSAFE_IO \
  static_cast<int>(NETIO_CONCURRENCY_HINT_ID \
      | NETIO_CONCURRENCY_HINT_LOCKING_SCHEDULER \
      | NETIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION)

// The special concurrency hint provides full thread safety.
#define NETIO_CONCURRENCY_HINT_SAFE \
  static_cast<int>(NETIO_CONCURRENCY_HINT_ID \
      | NETIO_CONCURRENCY_HINT_LOCKING_SCHEDULER \
      | NETIO_CONCURRENCY_HINT_LOCKING_REACTOR_REGISTRATION \
      | NETIO_CONCURRENCY_HINT_LOCKING_REACTOR_IO)
...
#if !defined(NETIO_CONCURRENCY_HINT_DEFAULT)
# define NETIO_CONCURRENCY_HINT_DEFAULT -1
#endif // !defined(NETIO_CONCURRENCY_HINT_DEFAULT)

// This #define may be overridden at compile time to specify a program-wide
// concurrency hint, used by the one-argument io_context constructor when
// passed a value of 1.
#if !defined(NETIO_CONCURRENCY_HINT_1)
# define NETIO_CONCURRENCY_HINT_1 1
#endif // !defined(NETIO_CONCURRENCY_HINT_DEFAULT)

#endif // NETIO_DETAIL_CONCURRENCY_HINT_HPP
