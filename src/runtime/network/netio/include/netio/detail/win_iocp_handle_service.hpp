#ifndef NETIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
#define NETIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
...
#include "netio/detail/win_iocp_handle_read_op.hpp"
#include "netio/detail/win_iocp_handle_write_op.hpp"
...
public:
  ...
  NETIO_DECL win_iocp_handle_service(execution_context& context);
      ...
      Handler& handler, const IoExecutor& io_ex)
  {
    ...
    NETIO_HANDLER_CREATION((iocp_service_.context(), *p.p, "handle", &impl,
...
private:
  ...
  size_t write_some(implementation_type& impl,
      const null_buffers& buffers, netio::error_code& ec);
  size_t write_some_at(implementation_type& impl, uint64_t offset,
      const null_buffers& buffers, netio::error_code& ec);
  ...
  NETIO_DECL size_t do_write(implementation_type& impl,
      uint64_t offset, const netio::const_buffer& buffer,
      netio::error_code& ec);

  // Helper function to start a write operation.
  NETIO_DECL void start_write_op(implementation_type& impl,
      uint64_t offset, const netio::const_buffer& buffer,
      operation* op);

  // Helper function to perform a synchronous write operation.
  NETIO_DECL size_t do_read(implementation_type& impl,
      uint64_t offset, const netio::mutable_buffer& buffer,
      netio::error_code& ec);

  // Helper function to start a read operation.
  NETIO_DECL void start_read_op(implementation_type& impl,
      uint64_t offset, const netio::mutable_buffer& buffer,
      operation* op);

  // Update the ID of the thread from which cancellation is safe.
  NETIO_DECL void update_cancellation_thread_id(implementation_type& impl);

  // Helper function to close a handle when the associated object is being
  // destroyed.
  NETIO_DECL void close_for_destruction(implementation_type& impl);
...
#endif // NETIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
