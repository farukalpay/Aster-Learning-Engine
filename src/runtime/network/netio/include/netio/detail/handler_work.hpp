#ifndef NETIO_DETAIL_HANDLER_WORK_HPP
#define NETIO_DETAIL_HANDLER_WORK_HPP
...
public:
  ...
  explicit handler_work_base(int, int, const executor_type& ex) noexcept
    ...
    : executor_(
        ex.target_type() == typeid(typename IoContext::executor_type)
          ? executor_type()
          : netio::prefer(ex, execution::outstanding_work.tracked))
#else // !defined(NETIO_NO_TYPEID)
    : executor_(netio::prefer(ex, execution::outstanding_work.tracked))
#endif // !defined(NETIO_NO_TYPEID)
  {
  ...
  handler_work(Handler& handler, const IoExecutor& io_ex) noexcept
    : base1_type(0, 0, io_ex),
      base2_type(base1_type::owns_work(),
          netio::get_associated_executor(handler, io_ex), io_ex)
  {
  }
...
#endif // NETIO_DETAIL_HANDLER_WORK_HPP
