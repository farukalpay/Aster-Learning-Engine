#ifndef NETIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
#define NETIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
...
#include "netio/detail/win_iocp_thread_info.hpp"
...
public:
  ...
  // Constructor.
  NETIO_DECL win_iocp_io_context(
      netio::execution_context& ctx, bool own_thread = true);

  // Construct as an internal scheduler.
  NETIO_DECL win_iocp_io_context(internal,
  ...
  NETIO_DECL netio::error_code register_handle(
      HANDLE handle, netio::error_code& ec);

  // Run the event loop until stopped or no more work.
  NETIO_DECL size_t run(netio::error_code& ec);

  // Run until stopped or one operation is performed.
  NETIO_DECL size_t run_one(netio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  NETIO_DECL size_t wait_one(long usec, netio::error_code& ec);

  // Poll for operations without blocking.
  NETIO_DECL size_t poll(netio::error_code& ec);

  // Poll for one operation without blocking.
  NETIO_DECL size_t poll_one(netio::error_code& ec);
  ...
  NETIO_DECL void post_deferred_completion(win_iocp_operation* op);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operations.
  NETIO_DECL void post_deferred_completions(
  ...
  NETIO_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  NETIO_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  NETIO_DECL void on_completion(win_iocp_operation* op,
      const netio::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  NETIO_DECL size_t do_one(DWORD msec,
      win_iocp_thread_info& this_thread, netio::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  NETIO_DECL static DWORD get_gqcs_timeout();
  ...
  netio::detail::thread timer_thread_;
...
#include "netio/detail/impl/win_iocp_io_context.hpp"
...
#endif // NETIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
