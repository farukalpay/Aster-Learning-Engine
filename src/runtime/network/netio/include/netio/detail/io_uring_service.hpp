#ifndef NETIO_DETAIL_IO_URING_SERVICE_HPP
#define NETIO_DETAIL_IO_URING_SERVICE_HPP
...
public:
  ...
  class io_queue : operation
  {
    ...
    NETIO_DECL operation* perform_io(int result);
    NETIO_DECL static void do_complete(void* owner, operation* base,
  ...
  struct io_object
  {
    ...
    NETIO_DECL io_object(bool locking, int spin_count);
  };
  ...
  // Constructor.
  NETIO_DECL io_uring_service(netio::execution_context& ctx);

  // Destructor.
  NETIO_DECL ~io_uring_service();
  ...
  NETIO_DECL void register_io_object(io_object*& io_obj);

  // Register an internal I/O object with io_uring.
  NETIO_DECL void register_internal_io_object(
      io_object*& io_obj, int op_type, io_uring_operation* op);

  // Register buffers with io_uring.
  NETIO_DECL void register_buffers(const ::iovec* v, unsigned n);

  // Unregister buffers from io_uring.
  NETIO_DECL void unregister_buffers();
  ...
  NETIO_DECL void start_op(int op_type, per_io_object_data& io_obj,
  ...
  // error.
  NETIO_DECL void cancel_ops(per_io_object_data& io_obj);
  ...
  NETIO_DECL void cancel_ops_by_key(per_io_object_data& io_obj,
  ...
  NETIO_DECL void deregister_io_object(per_io_object_data& io_obj);

  // Perform any post-deregistration cleanup tasks associated with the I/O
  // object.
  NETIO_DECL void cleanup_io_object(per_io_object_data& io_obj);
...
private:
  ...
  // Initialise the ring.
  NETIO_DECL void init_ring();

  // Register the eventfd descriptor for readiness notifications.
  NETIO_DECL void register_with_reactor();

  // Allocate a new I/O object.
  NETIO_DECL io_object* allocate_io_object();

  // Free an existing I/O object.
  NETIO_DECL void free_io_object(io_object* s);
  ...
  class submit_sqes_op : operation
  {
    ...
    NETIO_DECL submit_sqes_op(io_uring_service* s);
    NETIO_DECL static void do_complete(void* owner, operation* base,
        const netio::error_code& ec, std::size_t bytes_transferred);
  };
...
#include "netio/detail/impl/io_uring_service.hpp"
...
#endif // NETIO_DETAIL_IO_URING_SERVICE_HPP
