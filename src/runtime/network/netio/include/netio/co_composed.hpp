#ifndef NETIO_CO_COMPOSED_HPP
#define NETIO_CO_COMPOSED_HPP
...
#include "netio/detail/composed_work.hpp"
...
#  include "netio/detail/source_location.hpp"
...
using netio::detail::composed_io_executors;
using netio::detail::composed_work;
using netio::detail::composed_work_guard;
using netio::detail::get_composed_io_executor;
using netio::detail::make_composed_io_executors;
using netio::detail::recycling_allocator;
using netio::detail::throw_error;
...
struct co_composed_state_default_cancellation_on_suspend_impl<Executors,
    Handler, Return, R(netio::error_code, Args...), Signatures...>
{
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  using return_type = std::tuple<netio::error_code, Args...>;
  ...
  static constexpr void (*fn())(void*)
  {
    if constexpr ((is_constructible<Args>::value && ...))
    {
      return [](void* p)
      {
        ...
        std::move(handler)(
            netio::error_code(netio::error::operation_aborted),
            Args{}...);
      };
    }
    else
      ...
      return [](void* p)
      {
        ...
        std::move(handler)(
            std::make_exception_ptr(
              netio::system_error(
                netio::error::operation_aborted, "co_await")),
            Args{}...);
      };
    }
    else
...
private:
  ...
  void check_for_cancellation_on_transform()
  {
    if (throw_if_cancelled_ && !!cancelled())
      throw_error(netio::error::operation_aborted, "co_await");
  }
...
class co_composed_handler<Executors, Handler,
    Return, R(netio::error_code, Args...)>
  : public co_composed_handler_base<Executors, Handler, Return>
{
public:
  using co_composed_handler_base<Executors,
  ...
  using result_type = std::tuple<netio::error_code, args_type>;

  template <typename... T>
  void operator()(const netio::error_code& ec, T&&... args)
  {
...
public:
  ...
  template <NETIO_ASYNC_OPERATION Op>
  auto await_transform(Op&& op,
#if defined(NETIO_ENABLE_HANDLER_TRACKING)
# if defined(NETIO_HAS_SOURCE_LOCATION)
      netio::detail::source_location location
        = netio::detail::source_location::current(),
# endif // defined(NETIO_HAS_SOURCE_LOCATION)
#endif // defined(NETIO_ENABLE_HANDLER_TRACKING)
...
#if defined(NETIO_ENABLE_HANDLER_TRACKING)
# if defined(NETIO_HAS_SOURCE_LOCATION)
          , const netio::detail::source_location& location
# endif // defined(NETIO_HAS_SOURCE_LOCATION)
#endif // defined(NETIO_ENABLE_HANDLER_TRACKING)
        )
...
#if defined(NETIO_ENABLE_HANDLER_TRACKING)
# if defined(NETIO_HAS_SOURCE_LOCATION)
      netio::detail::source_location location_;
# endif // defined(NETIO_HAS_SOURCE_LOCATION)
#endif // defined(NETIO_ENABLE_HANDLER_TRACKING)
    };
...
struct coroutine_traits<void, C&,
    netio::detail::co_composed_state<Executors, Handler, Return>, Args...>
{
  using promise_type =
    netio::detail::co_composed_promise<Executors, Handler, Return>;
};
 ...
 * {
 *   return netio::async_initiate<
 *     CompletionToken, void(std::error_code)>(
 *       netio::co_composed(
 *         [](auto state, tcp::socket& socket) -> void
 *         {
 *           state.reset_cancellation_state(
 *             netio::enable_terminal_cancellation());
 *
 ...
 *                 make_error_code(netio::error::operation_aborted));
 *
 *             auto [e2, n2] =
 *               co_await netio::async_write(socket,
 *                 netio::buffer(data, n1));
 *
 *             if (e2)
