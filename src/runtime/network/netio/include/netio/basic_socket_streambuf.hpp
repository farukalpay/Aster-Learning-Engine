#ifndef NETIO_BASIC_SOCKET_STREAMBUF_HPP
#define NETIO_BASIC_SOCKET_STREAMBUF_HPP
...
public:
   ...
   * buffer.
   */
  const netio::error_code& error() const
  {
    return ec_;
  }
...
protected:
  int_type underflow()
  {
    ...
    ec_ = netio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(netio::buffer(get_buffer_) + putback_max);
      ...
      // Operation failed.
      if (ec_ != netio::error::would_block
          && ec_ != netio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = netio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = netio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != netio::error::in_progress
          && ec_ != netio::error::would_block)
        continue;
      ...
      ec_ = netio::error_code(connect_error,
...
#endif // NETIO_BASIC_SOCKET_STREAMBUF_HPP
