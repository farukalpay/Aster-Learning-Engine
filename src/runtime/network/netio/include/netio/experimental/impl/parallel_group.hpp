#ifndef NETIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
#define NETIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
...
struct parallel_group_completion_handler
{
  ...
  parallel_group_completion_handler(Handler&& h)
    : handler_(std::move(h)),
      executor_(
          netio::prefer(
            netio::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(netio::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(netio::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        netio::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      netio::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  netio::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
{
  typedef netio::cancellation_slot cancellation_slot_type;
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      netio::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        netio::cancellation_signal* sig = &signal_;
        netio::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...>> state_;
    netio::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...>>& state)
  {
    ...
    ex_type ex = netio::get_associated_executor(op);
    std::move(op)(
    ...
    enable_if_t<
      is_same<
        typename associated_executor<
          Op>::netio_associated_executor_is_unspecialised,
        void
      >::value
    >>
{
    ...
    std::tuple<Ops...>& ops, netio::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      netio::detail::recycling_allocator<state_type,
        netio::detail::thread_info_base::parallel_group_tag>(),
...
struct ranged_parallel_group_completion_handler
{
  ...
  ranged_parallel_group_completion_handler(Handler&& h,
    ...
    : handler_(std::move(h)),
      ...
      completion_order_(size, 0,
          NETIO_REBIND_ALLOC(Allocator, std::size_t)(allocator)),
      args_(NETIO_REBIND_ALLOC(Allocator, op_result_type)(allocator))
  {
  ...
  {
    this->invoke(
        netio::detail::make_index_sequence<
  ...
  void invoke(netio::detail::index_sequence<I...>)
  {
    ...
    vectors_type vectors{
        typename std::tuple_element<I, vectors_type>::type(
          NETIO_REBIND_ALLOC(Allocator, int)(allocator_))...};
  ...
  std::vector<std::size_t,
    NETIO_REBIND_ALLOC(Allocator, std::size_t)> completion_order_;
  std::deque<op_result_type,
    NETIO_REBIND_ALLOC(Allocator, op_result_type)> args_;
};
...
struct ranged_parallel_group_state
{
  ranged_parallel_group_state(Condition&& c, Handler&& h,
    ...
    : cancellations_requested_(size),
      ...
      cancellation_signals_(
          NETIO_REBIND_ALLOC(Allocator,
            netio::cancellation_signal)(allocator)),
      cancellation_condition_(std::move(c)),
  ...
  // The cancellation signals for each operation in the group.
  std::deque<netio::cancellation_signal,
    NETIO_REBIND_ALLOC(Allocator, netio::cancellation_signal)>
      cancellation_signals_;
...
#endif // NETIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
