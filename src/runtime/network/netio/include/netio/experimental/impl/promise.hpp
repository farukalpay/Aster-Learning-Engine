#ifndef NETIO_EXPERIMENTAL_IMPL_PROMISE_HPP
#define NETIO_EXPERIMENTAL_IMPL_PROMISE_HPP
...
template<typename Signature = void(),
    typename Executor = netio::any_io_executor,
...
struct promise_impl<void(Ts...), Executor, Allocator>
{
  ...
  void apply_impl(Func f, netio::detail::index_sequence<Idx...>)
  {
  ...
  void apply(Func f)
  {
    apply_impl(std::forward<Func>(f),
        netio::detail::make_index_sequence<sizeof...(Ts)>{});
  }
  ...
  struct completion_impl final : completion_base
  {
    ...
    void invoke(Ts&&... ts)
    {
      ...
      using alloc_t = typename std::allocator_traits<
        typename netio::decay<Alloc>::type>::template
  ...
  typename netio::aligned_storage<sizeof(void*) * 4,
  ...
  void set_completion(Alloc&& alloc, Handler&& handler)
  {
    ...
    using impl_t = completion_impl<
      typename netio::decay<Alloc>::type, Handler>;
    using alloc_t = typename std::allocator_traits<
      typename netio::decay<Alloc>::type>::template rebind_alloc<impl_t>;
  ...
  void complete_with_result_impl(netio::detail::index_sequence<Idx...>)
  {
    auto& result_type = *reinterpret_cast<promise_impl::result_type*>(&result);
  ...
  void cancel_impl_(std::exception_ptr*, T_*...)
  {
    complete(
        std::make_exception_ptr(
          netio::system_error(
            netio::error::operation_aborted)),
        T_{}...);
  }

  template<typename... T_>
  void cancel_impl_(netio::error_code*, T_*...)
  {
    complete(netio::error::operation_aborted, T_{}...);
  }
...
#endif // NETIO_EXPERIMENTAL_IMPL_PROMISE_HPP
