#ifndef NETIO_EXPERIMENTAL_IMPL_CORO_HPP
#define NETIO_EXPERIMENTAL_IMPL_CORO_HPP
...
#include "netio/experimental/detail/coro_completion_handler.hpp"
...
template <>
struct coro_error<netio::error_code>
{
  static netio::error_code invalid()
  {
    return netio::error::fault;
  }

  static netio::error_code cancelled()
  {
    return netio::error::operation_aborted;
  }

  static netio::error_code interrupted()
  {
    return netio::error::interrupted;
  }

  static netio::error_code done()
  {
    return netio::error::broken_pipe;
  }
};
...
struct coro_error<std::exception_ptr>
{
  static std::exception_ptr invalid()
  {
    return std::make_exception_ptr(
        netio::system_error(
          coro_error<netio::error_code>::invalid()));
  }
  ...
  static std::exception_ptr cancelled()
  {
    return std::make_exception_ptr(
        netio::system_error(
          coro_error<netio::error_code>::cancelled()));
  }
  ...
  static std::exception_ptr interrupted()
  {
    return std::make_exception_ptr(
        netio::system_error(
          coro_error<netio::error_code>::interrupted()));
  }
  ...
  static std::exception_ptr done()
  {
    return std::make_exception_ptr(
        netio::system_error(
          coro_error<netio::error_code>::done()));
  }
};
      ...
      -> coroutine_handle<>
    {
      ...
      if constexpr (!coro_promise<Y, R, E, A>::is_noexcept)
      {
            ...
            && hp.cancel->throw_if_cancelled_)
        {
          netio::detail::throw_error(
              netio::error::operation_aborted, "coro-cancelled");
        }
      }
      ...
      else
      {
        ...
        struct cancel_handler
        {
          ...
          {
            netio::dispatch(e, [ct, st = st]() mutable
            {
              auto & [sig, state] = *st;
              sig.emit(ct);
            });
      ...
      typename coro_traits<Yield, Return, Executor>::return_type>
{
  ...
  using cancellation_slot_type = netio::cancellation_slot;
  ...
  auto await_transform(this_coro::cancellation_state_t) const
  {
    struct exec_helper
    {
      const netio::cancellation_state& value;
      ...
      netio::cancellation_state await_resume() const noexcept
      {
        return value;
      }
    };
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
      ...
      std::true_type /* result is void */)
  {
    ...
    {
      if (!the_coro)
        return netio::post(exec,
            netio::append(std::move(h),
...
#endif // NETIO_EXPERIMENTAL_IMPL_CORO_HPP
