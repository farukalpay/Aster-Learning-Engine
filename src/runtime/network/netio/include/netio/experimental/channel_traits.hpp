#ifndef NETIO_EXPERIMENTAL_CHANNEL_TRAITS_HPP
#define NETIO_EXPERIMENTAL_CHANNEL_TRAITS_HPP
...
template <typename R>
struct channel_traits<R(netio::error_code)>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(netio::error_code);

  template <typename F>
  static void invoke_receive_cancelled(F f)
  {
    const netio::error_code e = error::channel_cancelled;
    static_cast<F&&>(f)(e);
  }

  typedef R receive_closed_signature(netio::error_code);

  template <typename F>
  static void invoke_receive_closed(F f)
  {
    const netio::error_code e = error::channel_closed;
    static_cast<F&&>(f)(e);
  }
};

template <typename R, typename... Args, typename... Signatures>
struct channel_traits<R(netio::error_code, Args...), Signatures...>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(netio::error_code, Args...);
  ...
  typedef R receive_closed_signature(netio::error_code, Args...);
...
struct channel_traits<R(std::exception_ptr)>
{
  ...
  static void invoke_receive_cancelled(F f)
  {
    const netio::error_code e = error::channel_cancelled;
    static_cast<F&&>(f)(
        std::make_exception_ptr(netio::system_error(e)));
  }
...
#endif // NETIO_EXPERIMENTAL_CHANNEL_TRAITS_HPP
