#ifndef NETIO_EXPERIMENTAL_BASIC_CHANNEL_HPP
#define NETIO_EXPERIMENTAL_BASIC_CHANNEL_HPP
...
#include "netio/experimental/detail/channel_send_functions.hpp"
#include "netio/experimental/detail/channel_service.hpp"
...
private:
  ...
  typedef detail::channel_service<netio::detail::null_mutex> service_type;
  ...
  template <typename... PayloadSignatures,
      NETIO_COMPLETION_TOKEN_FOR(PayloadSignatures...) CompletionToken>
  auto do_async_receive(
      netio::detail::completion_payload<PayloadSignatures...>*,
...
public:
  ...
  basic_channel(const executor_type& ex, std::size_t max_buffer_size = 0)
    : service_(&netio::use_service<service_type>(
  ...
  basic_channel(ExecutionContext& context, std::size_t max_buffer_size = 0,
      ...
      > = defaulted_constraint())
    : service_(&netio::use_service<service_type>(context)),
      impl_(),
   ...
   * @c netio::experimental::error::channel_cancelled. Outstanding receive
  ...
  template <typename... Args,
      NETIO_COMPLETION_TOKEN_FOR(void (netio::error_code))
        CompletionToken NETIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
  ...
  template <typename CompletionToken
      NETIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
  auto async_receive(
      CompletionToken&& token
...
private:
  ...
  {
    return netio::query(t, execution::context);
  }
  ...
  public:
    ...
    {
      netio::detail::non_const_lvalue<SendHandler> handler2(handler);
  ...
  public:
    ...
    {
      netio::detail::non_const_lvalue<ReceiveHandler> handler2(handler);
...
#endif // NETIO_EXPERIMENTAL_BASIC_CHANNEL_HPP
