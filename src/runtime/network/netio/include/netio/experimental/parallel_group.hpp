#ifndef NETIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
#define NETIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
...
#include "netio/experimental/cancellation_condition.hpp"
...
{
  typedef netio::detail::array<std::size_t, N> order_type;
...
void parallel_group_launch(Condition cancellation_condition, Handler handler,
    std::tuple<Ops...>& ops, netio::detail::index_sequence<I...>);
...
struct ranged_parallel_group_signature<R(Args...), Allocator>
{
  typedef std::vector<std::size_t,
    NETIO_REBIND_ALLOC(Allocator, std::size_t)> order_type;
  typedef R raw_type(
      std::vector<Args, NETIO_REBIND_ALLOC(Allocator, Args)>...);
...
/**
 * See the documentation for netio::experimental::make_parallel_group for
 * a usage example.
 */
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(Handler&& h, Condition&& c, std::tuple<Ops...>&& ops) const
    {
      detail::parallel_group_launch(
          ...
          ops, netio::detail::index_sequence_for<Ops...>());
    }
  };
...
public:
   ...
   * operations, it must return a netio::cancellation_type value other
   * than <tt>netio::cancellation_type::none</tt>.
   *
   ...
   *
   * @li netio::experimental::wait_for_all
   * @li netio::experimental::wait_for_one
   * @li netio::experimental::wait_for_one_error
   * @li netio::experimental::wait_for_one_success
   */
  template <typename CancellationCondition,
      NETIO_COMPLETION_TOKEN_FOR(signature) CompletionToken>
  auto async_wait(CancellationCondition cancellation_condition,
      CompletionToken&& token)
    -> decltype(
      netio::async_initiate<CompletionToken, signature>(
        declval<initiate_async_wait>(), token,
        std::move(cancellation_condition), std::move(ops_)))
  {
    return netio::async_initiate<CompletionToken, signature>(
...
/**
 * For example:
 * @code netio::experimental::make_parallel_group(
 *    in.async_read_some(netio::buffer(data)),
 *    timer.async_wait()
 *  ).async_wait(
 *    netio::experimental::wait_for_all(),
 *    [](
 ...
 *    [&](auto token)
 *    {
 *      return in.async_read_some(netio::buffer(data), token);
 *    },
 *    [&](auto token)
 *    {
 ...
 */
template <typename... Ops>
NETIO_NODISCARD inline parallel_group<Ops...>
...
public:
      ...
      allocator_(allocator)
  {
    if (range_.empty())
    {
      ...
      netio::detail::throw_exception(e);
    }
  }
 ...
 * @code
 * using op_type =
 *   decltype(socket1.async_read_some(netio::buffer(data1)));
 *
 * std::vector<op_type> ops;
 * ops.push_back(socket1.async_read_some(netio::buffer(data1)));
 * ops.push_back(socket2.async_read_some(netio::buffer(data2)));
 *
 * netio::experimental::make_parallel_group(ops).async_wait(
 *     netio::experimental::wait_for_all(),
 *     [](
...
NETIO_NODISCARD inline ranged_parallel_group<decay_t<Range>>
 ...
 * ops.push_back(socket2.async_read_some(netio::buffer(data2)));
 *
 * netio::experimental::make_parallel_group(
 *     std::allocator_arg_t,
 *     my_allocator,
 *     ops
 *   ).async_wait(
...
NETIO_NODISCARD inline ranged_parallel_group<decay_t<Range>, Allocator>
...
#include "netio/experimental/impl/parallel_group.hpp"

#endif // NETIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
