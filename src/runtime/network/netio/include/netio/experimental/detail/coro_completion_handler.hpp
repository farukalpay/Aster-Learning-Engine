#ifndef NETIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
#define NETIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
...
auto coro_interpret_result(
    std::tuple<netio::error_code, Args...>&& args)
{
  if (std::get<0>(args))
    netio::detail::throw_exception(
        netio::system_error(std::get<0>(args)));
...
inline auto coro_interpret_result(
    std::tuple<netio::error_code>&& args)
{
  if (std::get<0>(args))
    netio::detail::throw_exception(
        netio::system_error(std::get<0>(args)));
...
template <typename Arg>
auto coro_interpret_result(std::tuple<netio::error_code, Arg>&& args)
{
  if (std::get<0>(args))
    netio::detail::throw_exception(
        netio::system_error(std::get<0>(args)));
...
#endif // NETIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
