#ifndef NETIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
#define NETIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
...
#include "netio/detail/completion_payload.hpp"
#include "netio/detail/completion_payload_handler.hpp"
#include "netio/detail/mutex.hpp"
#include "netio/detail/op_queue.hpp"
#include "netio/execution_context.hpp"
#include "netio/experimental/detail/channel_receive_op.hpp"
#include "netio/experimental/detail/channel_send_op.hpp"
#include "netio/experimental/detail/has_signature.hpp"
...
class channel_service
  : public netio::detail::execution_context_service_base<
...
public:
  ...
  struct base_implementation_type
  {
    ...
    netio::detail::op_queue<channel_operation> waiters_;
  ...
  // Constructor.
  channel_service(netio::execution_context& ctx);
      ...
      Handler& handler, const IoExecutor& io_ex)
  {
    ...
    NETIO_HANDLER_CREATION((this->context(), *p.p,
...
private:
  ...
  struct post_receive
  {
    ...
    {
      op_->post(
          netio::detail::completion_message<Signature>(0,
  ...
  typedef conditional_t<
      ...
      conditional_t<
        has_signature<
          ...
          Signatures...
        >::value,
        netio::detail::completion_payload<Signatures...>,
        netio::detail::completion_payload<
          Signatures...,
          typename traits_type::receive_closed_signature
        >
      >,
  ...
  : channel_service::base_implementation_type
{
  ...
  typedef conditional_t<
      ...
      conditional_t<
        has_signature<
          ...
          R()
        >::value,
        netio::detail::completion_payload<R()>,
        netio::detail::completion_payload<
          R(),
          typename traits_type::receive_closed_signature
        >
  ...
  payload_type buffer_front()
  {
    return payload_type(netio::detail::completion_message<R()>(0));
  }
...
struct channel_service<Mutex>::implementation_type<
    Traits, R(netio::error_code)>
  : channel_service::base_implementation_type
{
  // The traits type associated with the channel.
  typedef typename Traits::template rebind<R(netio::error_code)>::other
    traits_type;
  ...
  typedef conditional_t<
      ...
      conditional_t<
        has_signature<
          ...
          R(netio::error_code)
        >::value,
        netio::detail::completion_payload<R(netio::error_code)>,
        netio::detail::completion_payload<
          R(netio::error_code),
          typename traits_type::receive_closed_signature
        >
  ...
  void buffer_push(payload_type payload)
  {
    ...
    {
      netio::error_code value{last.value_};
...
private:
  ...
  {
    netio::error_code value_;
    std::size_t count_;
  };

  struct value_handler
  {
    netio::error_code& target_;

    template <typename... Args>
    void operator()(const netio::error_code& value, Args&&...)
    {
      target_ = value;
    }
  };
...
#include "netio/experimental/detail/impl/channel_service.hpp"

#endif // NETIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
