#ifndef NETIO_EXPERIMENTAL_CORO_HPP
#define NETIO_EXPERIMENTAL_CORO_HPP
...
#include "netio/experimental/detail/coro_promise_allocator.hpp"
#include "netio/experimental/detail/partial_promise.hpp"
...
struct coro
{
  /// The traits of the coroutine. See netio::experimental::coro_traits
  /// for details.
  ...
  ~coro()
  {
    if (coro_ != nullptr)
    {
      ...
      if (handle)
        netio::dispatch(coro_->get_executor(), destroyer{handle});
    }
  }
...
template<typename T, typename Executor = netio::any_io_executor,
    typename Allocator = std::allocator<void>>
...
#include "netio/experimental/impl/coro.hpp"

#endif // NETIO_EXPERIMENTAL_CORO_HPP
