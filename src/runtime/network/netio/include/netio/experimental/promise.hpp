#ifndef NETIO_EXPERIMENTAL_PROMISE_HPP
#define NETIO_EXPERIMENTAL_PROMISE_HPP
...
#include "netio/experimental/impl/promise.hpp"
 ...
 * @code
 * awaitable<void> read_write_some(netio::ip::tcp::socket & sock,
 *     netio::mutable_buffer read_buf, netio::const_buffer to_write)
 * {
 *   auto p = netio::async_read(read_buf,
 *       netio::experimental::use_promise);
 *   co_await netio::async_write_some(to_write);
 *   co_await p;
 * }
 * @endcode
 */
...
#endif // defined(GENERATING_DOCUMENTATION)
{
  ...
  template <NETIO_COMPLETION_TOKEN_FOR(void(Ts...)) CompletionToken>
  inline NETIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void(Ts...))
...
private:
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(WaitHandler&& handler) const
    {
      ...
      if (self_->done)
      {
        auto exec = netio::get_associated_executor(
...
#endif // NETIO_EXPERIMENTAL_PROMISE_HPP
