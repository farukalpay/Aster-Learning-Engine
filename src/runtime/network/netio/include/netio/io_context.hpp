#ifndef NETIO_IO_CONTEXT_HPP
#define NETIO_IO_CONTEXT_HPP
...
#include "netio/detail/wrapped_handler.hpp"
...
# include "netio/detail/winsock_init.hpp"
  ...
  || defined(__osf__)
# include "netio/detail/signal_init.hpp"
#endif
 ...
 *
 * @li netio::ip::tcp::socket
 * @li netio::ip::tcp::acceptor
 * @li netio::ip::udp::socket
 * @li netio::deadline_timer.
 *
 ...
 *
 * To submit functions to the io_context, use the @ref netio::dispatch,
 * @ref netio::post or @ref netio::defer free functions.
 *
 * For example:
 *
 * @code void my_task()
 ...
 * netio::post(io_context, my_task);
 *
 * // Submit a lambda object to the io_context.
 * netio::post(io_context,
 *     []()
 *     {
 *       ...
 *     });
 ...
 * netio::executor_work_guard<io_context::executor_type>:
 *
 * @code netio::io_context io_context;
 * netio::executor_work_guard<netio::io_context::executor_type>
 *   = netio::make_work_guard(io_context);
 * ... @endcode
 *
...
public:
   ...
   */
  NETIO_DECL explicit io_context(int concurrency_hint);
   ...
   */
  NETIO_DECL explicit io_context(
   ...
   * @return The number of handlers that were executed.
   */
  NETIO_DECL count_type poll_one();
   ...
   */
  NETIO_DECL void restart();

#if !defined(NETIO_NO_DEPRECATED)
  /// (Deprecated: Use netio::bind_executor().) Create a new handler that
   ...
   * @code netio::dispatch(io_context,
   *     boost::bind(f, a1, ... an)); @endcode
   */
  template <typename Handler>
  NETIO_DEPRECATED_MSG("Use netio::bind_executor()")
...
private:
   ...
   * @code auto ex1 = my_io_context.get_executor();
   * auto ex2 = netio::require(ex1,
   *     netio::execution::outstanding_work.tracked); @endcode
   */
  constexpr basic_executor_type<Allocator,
      NETIO_UNSPECIFIED(Bits | outstanding_work_tracked)>
   ...
   * @code auto ex1 = my_io_context.get_executor();
   * auto ex2 = netio::require(ex1,
   *     netio::execution::outstanding_work.untracked); @endcode
   */
  constexpr basic_executor_type<Allocator,
      NETIO_UNSPECIFIED(Bits & ~outstanding_work_tracked)>
...
private:
   ...
   *     == netio::execution::inline_exception_handling.capture)
   *   ... @endcode
   */
   ...
   * For example:
   * @code auto ex = my_io_context.get_executor();
   * netio::io_context& ctx = netio::query(
   ...
   * For example:
   * @code auto ex = my_io_context.get_executor();
   * if (netio::query(ex, netio::execution::outstanding_work)
   *       == netio::execution::outstanding_work.tracked)
   *   ... @endcode
   */
...
public:
  ...
  netio::io_context& get_io_context();

private:
  /// Destroy all user-defined handler objects owned by the service.
  NETIO_DECL virtual void shutdown();
   ...
   */
  NETIO_DECL virtual void notify_fork(
...
protected:
   ...
   */
  NETIO_DECL service(netio::io_context& owner);

  /// Destructor.
  NETIO_DECL virtual ~service();
};
...
class service_base
  : public netio::io_context::service
{
public:
  static netio::detail::service_id<Type> id;

  // Constructor.
  service_base(netio::io_context& io_context)
    : netio::io_context::service(io_context)
  {
  }
};

template <typename Type>
netio::detail::service_id<Type> service_base<Type>::id;
...
struct equality_comparable<
    netio::io_context::basic_executor_type<Allocator, Bits>
  >
{
  ...
  >
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = false;
  typedef netio::io_context::basic_executor_type<
...
struct require_member<
    netio::io_context::basic_executor_type<Allocator, Bits>,
    netio::execution::outstanding_work_t::tracked_t
  > : netio::detail::io_context_bits
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = false;
  typedef netio::io_context::basic_executor_type<
...
struct require_member<
    netio::io_context::basic_executor_type<Allocator, Bits>,
    netio::execution::outstanding_work_t::untracked_t
  > : netio::detail::io_context_bits
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = false;
...
struct query_static_constexpr_member<
    ...
    typename netio::enable_if<
      netio::is_convertible<
        Property,
        netio::execution::outstanding_work_t
      >::value
    >::type
  > : netio::detail::io_context_bits
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = true;
  typedef netio::execution::outstanding_work_t result_type;
  ...
  >
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = true;
  typedef netio::execution::inline_exception_handling_t::capture_t
    result_type;
  ...
  >
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = true;
  typedef netio::io_context& result_type;
};
...
#include "netio/impl/io_context.hpp"
...
#if !defined(NETIO_NO_EXTENSIONS)
# if defined(NETIO_STRAND_HPP)
#  include "netio/io_context_strand.hpp"
# endif // defined(NETIO_STRAND_HPP)
#endif // !defined(NETIO_NO_EXTENSIONS)

#endif // NETIO_IO_CONTEXT_HPP
