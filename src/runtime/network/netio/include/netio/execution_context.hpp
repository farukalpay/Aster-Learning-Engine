#ifndef NETIO_EXECUTION_CONTEXT_HPP
#define NETIO_EXECUTION_CONTEXT_HPP
...
public:
   ...
   */
  NETIO_DECL explicit execution_context(
...
public:
   ...
   *
   * @throws netio::system_error Thrown on failure. If the notification
   ...
   */
  NETIO_DECL void notify_fork(fork_event event);
   ...
   * constructor.
   *
   * @throws netio::service_already_exists Thrown if a service of the
   ...
   * given type is already present in the execution_context.
   *
   * @throws netio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  NETIO_DECL explicit execution_context(allocator_impl_base* alloc);
  NETIO_DECL execution_context(allocator_impl_base* alloc,
      const service_maker& initial_services);
...
protected:
  NETIO_DECL virtual ~allocator_impl_base();
};
...
protected:
   ...
   */
  NETIO_DECL service(execution_context& owner);
...
protected:
  /// Destructor.
  NETIO_DECL virtual ~service_maker();
};
...
{
public:
  NETIO_DECL service_already_exists();
};
...
{
public:
  NETIO_DECL invalid_service_owner();
};
