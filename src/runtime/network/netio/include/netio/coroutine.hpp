#ifndef NETIO_COROUTINE_HPP
#define NETIO_COROUTINE_HPP
 ...
 *
 * @code #include <netio/yield.hpp>@endcode
 *
 * and may conversely be undefined as follows:
 *
 * @code #include <netio/unyield.hpp>@endcode
 *
 * <b>reenter</b>
 *
 ...
 * execution using something like netio::post().
 *
 ...
 *
 * @li @c NETIO_CORO_REENTER instead of @c reenter
 * @li @c NETIO_CORO_YIELD instead of @c yield
 * @li @c NETIO_CORO_FORK instead of @c fork
 */
class coroutine
{
public:
...
#if !defined(NETIO_CORO_VALUE_INIT)
# if defined(_MSC_VER)
#  define NETIO_CORO_VALUE_INIT __COUNTER__
# else // defined(_MSC_VER)
#  define NETIO_CORO_VALUE_INIT __LINE__
# endif // defined(_MSC_VER)
#endif // !defined(NETIO_CORO_VALUE_INIT)

#define NETIO_CORO_REENTER(c) \
  if (constexpr ::netio::detail::coroutine_base_value \
      _coro_base_value = NETIO_CORO_VALUE_INIT) \
  { \
  } \
  else switch (::netio::detail::coroutine_ref _coro_value = c) \
...
#define NETIO_CORO_YIELD_IMPL(n) \
  for (_coro_value = (n);;) \
    if (_coro_value == 0) \
    { \
      case (n): ; \
...
# define NETIO_CORO_YIELD \
  NETIO_CORO_YIELD_IMPL( \
    NETIO_CORO_VALUE_INIT + 1 - _coro_base_value.get())

# define NETIO_CORO_FORK \
  NETIO_CORO_FORK_IMPL( \
    NETIO_CORO_VALUE_INIT + 1 - _coro_base_value.get())

#endif // NETIO_COROUTINE_HPP
