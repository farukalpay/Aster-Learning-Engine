...
#if PROFILE_RUNTIME_ENABLE_GPU_VULKAN
#include <vulkan/vulkan.h>

#include "profile_runtime_core.h"
#include "profile_runtime_gpu.h"

#define PROFILE_RUNTIME_VK_CHECK(args) do { VkResult __hr = args; PROFILE_RUNTIME_ASSERT(__hr == VK_SUCCESS, "Failed check"); (void)__hr; } while(false);
...
namespace ProfileRuntime
{
	...
	void GPUProfilerVulkan::InitDevice(VkDevice* devices, VkPhysicalDevice* physicalDevices, VkQueue* cmdQueues, uint32_t* cmdQueuesFamily, uint32_t nodeCount, const VulkanFunctions* functions)
	{
		...
		for (uint32_t i = 0; i < nodeCount; ++i)
		{
			...
			PROFILE_RUNTIME_ASSERT(r == VK_SUCCESS, "Failed");
			(void)r;
	...
	void GPUProfilerVulkan::ResolveTimestamps(VkCommandBuffer commandBuffer, uint32_t startIndex, uint32_t count)
	{
		if (count)
		{
			...
			PROFILE_RUNTIME_VK_CHECK((VkResult)(*vulkanFunctions.vkGetQueryPoolResults)(payload->device, payload->queryPool, startIndex, count, 8 * count, &nodes[currentNode]->queryGpuTimestamps[startIndex], 8, VK_QUERY_RESULT_64_BIT));
	...
	{
		PROFILE_RUNTIME_CATEGORY("GPUProfilerVulkan::Flip", Category::Debug);
		...
		if (currentState == STATE_RUNNING)
		{
			...
			commandBufferBeginInfo.pInheritanceInfo = 0;
			commandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
			PROFILE_RUNTIME_VK_CHECK((VkResult)(*vulkanFunctions.vkBeginCommandBuffer)(commandBuffer, &commandBufferBeginInfo));
			...
			PROFILE_RUNTIME_VK_CHECK((VkResult)(*vulkanFunctions.vkEndCommandBuffer)(commandBuffer));
			VkSubmitInfo submitInfo = {};
			submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
			submitInfo.pNext = nullptr;
			submitInfo.waitSemaphoreCount = 0;
			...
			submitInfo.signalSemaphoreCount = 0;
			submitInfo.pSignalSemaphores = nullptr;
			PROFILE_RUNTIME_VK_CHECK((VkResult)(*vulkanFunctions.vkQueueSubmit)(queue, 1, &submitInfo, fence));
...
namespace ProfileRuntime
{
	void InitGpuVulkan(VkDevice* /*vkDevices*/, VkPhysicalDevice* /*vkPhysicalDevices*/, VkQueue* /*vkQueues*/, uint32_t* /*cmdQueuesFamily*/, uint32_t /*numQueues*/, const VulkanFunctions* /*functions*/)
	{
		PROFILE_RUNTIME_FAILED("PROFILE_RUNTIME_ENABLE_GPU_VULKAN is disabled! Can't initialize GPU Profiler!");
	}
}
