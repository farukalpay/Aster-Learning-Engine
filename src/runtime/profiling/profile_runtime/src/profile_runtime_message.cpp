...
class MessageFactory
{
	...
	MessageFactory()
	{
		...
		{
			PROFILE_RUNTIME_ASSERT(factory[msg] != nullptr, "Message is not registered to factory");
		}
	}
public:
	...
	IMessage* Create(InputDataStream& str)
	{
		...
		PROFILE_RUNTIME_VERIFY( messageType < IMessage::COUNT && factory[messageType] != nullptr, "Unknown message type!", return nullptr )

		IMessage* result = factory[messageType](str);

		if (header.length + str.Length() != length)
		{
			PROFILE_RUNTIME_FAILED("Message Stream is corrupted! Invalid Protocol?")
			return nullptr;
		}
