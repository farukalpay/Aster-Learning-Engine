...
#include "profile_runtime_miniz.h"
...
class Wsa
{
	...
	Wsa()
	{
		...
		PROFILE_RUNTIME_ASSERT(isInitialized, "Can't initialize WSA");
	}
...
#else 
#if defined(PROFILE_RUNTIME_OSX) || defined(PROFILE_RUNTIME_LINUX)
...
public:
	Socket() : acceptSocket((TcpSocket)-1), listenSocket((TcpSocket)-1)
	{
		...
		PROFILE_RUNTIME_ASSERT(IsValidSocket(listenSocket), "Can't create socket");
	...
	void Listen()
	{
		...
		if (result != 0)
		{
			PROFILE_RUNTIME_FAILED("Can't start listening");
		}
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
struct ProfileRuntimeHeader
{
	uint32_t magic;
	uint16_t version;
	uint16_t flags;

	static const uint32_t PROFILE_RUNTIME_MAGIC = 0xB50FB50Fu;
	static const uint16_t PROFILE_RUNTIME_VERSION = 0;
	enum Flags : uint16_t
	{
		IsZip = 1 << 0,
		IsMiniz = 1 << 1,
	...
	ProfileRuntimeHeader() : magic(PROFILE_RUNTIME_MAGIC), version(PROFILE_RUNTIME_VERSION), flags(0) {}
};
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Server::Server(short port) : socket(Memory::New<Socket>()), saveCb(nullptr)
{
	...
	{
		PROFILE_RUNTIME_FAILED("Failed to bind a socket! Most probably the port is blocked by anti-virus! Change the port and verify that your game has enough permissions to communicate over the TCP\IP.");
	}
	else
	{
		socket->Listen();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#if PROFILE_RUNTIME_ENABLE_COMPRESSION
struct ZLibCompressor
{
	...
	void Init()
	{
		...
		{
			PROFILE_RUNTIME_FAILED("deflateInit failed!");
		}
	}
	...
	void Compress(const char* data, size_t size, CompressCb cb, bool finish = false)
	{
		...
		while (stream.avail_in || finish)
		{
			...
			if (status != Z_OK)
			{
				PROFILE_RUNTIME_FAILED("Copmression failed!");
				break;
			}
		}
	}
	...
	void Finish(CompressCb cb)
	{
		...
		if (status != Z_OK)
		{
			PROFILE_RUNTIME_FAILED("deflateEnd failed!");
		}
		buffer.clear();
...
void Server::SendStart()
{
	if (saveCb != nullptr)
	{
		ProfileRuntimeHeader header;
#if PROFILE_RUNTIME_ENABLE_COMPRESSION
		ZLibCompressor::Get().Init();
		header.flags |= ProfileRuntimeHeader::IsMiniz;
#endif
