...
#define DECLARE_ETW (!PROFILE_RUNTIME_PC)
...
struct Process_TypeGroup1
{
	...
	{
		PROFILE_RUNTIME_ASSERT((pEvent->EventHeader.Flags & EVENT_HEADER_FLAG_64_BIT_HEADER) != 0, "32-bit is not supported! Disable PROFILE_RUNTIME_ENABLE_TRACING on 32-bit platform if needed!");
...
{
	Memory::InitThread();
	Core::Get().RegisterThreadDescription(ThreadDescription("[ProfileRuntime] ETW", GetCurrentThreadId(), GetCurrentProcessId()));
	ETW* etw = (ETW*)parameter;
	ULONG status = ProcessTrace(&etw->openedHandle, 1, 0, 0);
	PROFILE_RUNTIME_UNUSED(status);
	return 0;
}
	...
	, isActive(false)
{
	...
	PROFILE_RUNTIME_ASSERT(g_ETW == nullptr, "Can't create more than one ETW session");
	g_ETW = this;
}
	...
	if (!isActive)
	{
		...
		*/
		if (callstackCountSamplesCount > 0)
		{
			...
			if (status != ERROR_SUCCESS)
			{
				PROFILE_RUNTIME_FAILED("TraceSetInformation - failed");
				return CaptureStatus::ERR_TRACER_FAILED;
			}
		}
		...
		if (mode & Mode::AUTOSAMPLING)
		{
			...
			PROFILE_RUNTIME_ASSERT(status == ERROR_SUCCESS, "TraceSetInformation - failed");
		}
		...
		{
			PROFILE_RUNTIME_FAILED("OpenTrace - failed");
			return CaptureStatus::ERR_TRACER_FAILED;
		}
...
// Symbol Resolving
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#define USE_DBG_HELP (PROFILE_RUNTIME_PC)
...
#pragma warning (pop)
	...
	{
		PROFILE_RUNTIME_FAILED("Can't query System Module Information!");
	}
