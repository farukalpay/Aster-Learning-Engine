...
namespace ProfileRuntime
{
	...
	public:
		static PROFILE_RUNTIME_INLINE void* Alloc(size_t size)
		{
			size_t totalSize = size + sizeof(Header);
			void *ptr = allocate(totalSize);
			PROFILE_RUNTIME_VERIFY(ptr, "Can't allocate memory", return nullptr);
		...
		static PROFILE_RUNTIME_INLINE void Free(void* p)
		{
			if (p != nullptr)
			{
		...
		static PROFILE_RUNTIME_INLINE size_t GetAllocatedSize()
		{
	...
	class MemoryPool
	{
		...
		PROFILE_RUNTIME_INLINE void AddChunk()
		{
			index = 0;
			if (!chunk || !chunk->next)
			{
	...
	public:
		...
		PROFILE_RUNTIME_INLINE T& Add(const T& item)
		{
			return Add() = item;
		}

		PROFILE_RUNTIME_INLINE T* AddRange(const T* items, size_t count, bool allowOverlap = true)
		{
		...
		PROFILE_RUNTIME_INLINE T* TryAdd(int count)
		{
			if (index + count <= SIZE)
			{
		...
		PROFILE_RUNTIME_INLINE T* Front()
		{
			return !IsEmpty() ? &root->data[0] : nullptr;
		}

		PROFILE_RUNTIME_INLINE size_t Size() const
		{
			if (root == nullptr)
				return 0;
		...
		PROFILE_RUNTIME_INLINE bool IsEmpty() const
		{
			return (chunk == nullptr) || (chunk == root && index == 0);
		}

		PROFILE_RUNTIME_INLINE void Clear(bool preserveMemory = true)
		{
			if (!preserveMemory)
			{
				if (root)
