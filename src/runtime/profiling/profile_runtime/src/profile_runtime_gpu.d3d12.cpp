...
#define PROFILE_RUNTIME_CHECK(args) do { HRESULT __hr = args; (void)__hr; PROFILE_RUNTIME_ASSERT(__hr == S_OK, "Failed check"); } while(false);
...
namespace ProfileRuntime
{
	...
	void GPUProfilerD3D12::InitDevice(ID3D12Device* pDevice, ID3D12CommandQueue** pCommandQueues, uint32_t numCommandQueues)
	{
		...
		PROFILE_RUNTIME_CHECK(device->CreateCommittedResource(
			&heapDesc,
		...
		PROFILE_RUNTIME_CHECK(CreateDXGIFactory2(0, IID_PPV_ARGS(&factory)));
		...
		wcstombs_s(deviceName, desc.Description, PROFILE_RUNTIME_ARRAY_SIZE(deviceName) - 1);
	...
	void GPUProfilerD3D12::InitNodeInternal(const char* nodeName, uint32_t nodeIndex, ID3D12CommandQueue* pCmdQueue)
	{
		...
		PROFILE_RUNTIME_CHECK(device->CreateQueryHeap(&queryHeapDesc, IID_PPV_ARGS(&node->queryHeap)));

		PROFILE_RUNTIME_CHECK(device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&node->syncFence)));

		for (Frame& frame : node->frames)
		{
			PROFILE_RUNTIME_CHECK(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&frame.commandAllocator)));
			PROFILE_RUNTIME_CHECK(device->CreateCommandList(1u << nodeIndex, D3D12_COMMAND_LIST_TYPE_DIRECT, frame.commandAllocator, nullptr, IID_PPV_ARGS(&frame.commandList)));
			PROFILE_RUNTIME_CHECK(frame.commandList->Close());
		}
	}
	...
	{
		PROFILE_RUNTIME_CATEGORY("GPUProfilerD3D12::Flip", Category::Debug);
		...
		if (currentState == STATE_RUNNING)
		{
			...
			{
				PROFILE_RUNTIME_ASSERT(queryEnd - queryBegin <= MAX_QUERIES_COUNT, "Too many queries in one frame? Increase GPUProfiler::MAX_QUERIES_COUNT to fix the problem!");
...
namespace ProfileRuntime
{
	void InitGpuD3D12(ID3D12Device* /*device*/, ID3D12CommandQueue** /*cmdQueues*/, uint32_t /*numQueues*/)
	{
		PROFILE_RUNTIME_FAILED("PROFILE_RUNTIME_ENABLE_GPU_D3D12 is disabled! Can't initialize GPU Profiler!");
	}
}

#endif //PROFILE_RUNTIME_ENABLE_GPU_D3D12
