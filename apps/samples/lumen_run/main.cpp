...
int main(int argc, char **argv) {
  try {
    ...
    if (learning_proof_run) {
      ...
      const aster::LumenLearningProofResult proof = aster::runLumenMiningLearningProof(
          authoring, std::move(*learning_session), learning_out);
    ...
    const auto pump_learning = [&]() {
      ...
      if (!candidates.empty() &&
          learning_session->selectScaffold(candidates.front().scaffold_id)) {
